...
export async function transcribeVoice(fileId) {
  ...
  if (groqRes.ok && groqData.text?.trim()) return groqData.text;

  console.error('TELEGRAM_GROQ_VOICE failed, trying Gemini fallback:', groqRes.status);
  // فشل Groq Whisper -> نجرب Gemini بنفس الملف الصوتي قبل ما نرجّع فاضي
...
export async function transcribeAudioBase64(audioBase64, mimeType = 'audio/webm') {
  ...
  try {
    ...
    console.error('DASHBOARD_GROQ_VOICE failed, trying Gemini fallback:', response.status);

...
// مفتاح اليوم الحالي (بتوقيت القاهرة، متضبط في config.js) — بيتغيّر كل نص الليل، فبيدّي كل يوم من
// الـ3 أيام تجربة عداده الخاص من الصفر تلقائيًا (زي ما الشهر بيعمل مع المشترك المدفوع بالظبط).
function currentTrialDayKey() {
  const now = new Date();
  return `trial-${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
}
...
async function resolveUsageContext(userId) {
  ...
  if (trial) {
    // period يومي مش ثابت طول الـ3 أيام — عشان الحدود (15 فويس/15 نص/3 فاتورة) تتطبّق لكل يوم لوحده.
    // isInTrial() برضو هي البوابة اللي بتقفل الوصول تمامًا بعد ما الـ3 أيام تخلص وتفرض الاشتراك.
    return { period: currentTrialDayKey(), limits: USAGE_LIMITS.trial, isTrial: true };
  }
  return null;
}
...
export const checkTextUsage = (userId) => checkAndIncrement(userId, 'text');
export const checkStatementUsage = (userId) => checkAndIncrement(userId, 'statement');
...
export const refundStatementUsage = (userId) => refundUsage(userId, 'statement');

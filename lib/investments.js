...
export async function addPortfolioAsset(userId, { name, subLabel, amount, alertThresholdPct }) {
  ...
  const { data, error } = await supabase
    .from('portfolio_assets')
    .insert({ telegram_user_id: userId, name: cleanName, sub_label: (subLabel || '').trim() || null, amount: cleanAmount, alert_threshold_pct: hasThreshold ? cleanThreshold : null })
    .select('*')
    .single();
...
export async function updatePortfolioAsset(userId, assetId, { name, subLabel, amount, alertThresholdPct }) {

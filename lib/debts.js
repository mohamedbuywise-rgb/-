...
export async function getFullDebtReportData(userId) {
  const { data: debts, error: debtsError } = await supabase
    .from('debts')
    .select('person_name, amount, currency_code, direction, note, created_at, is_repayment')
    .eq('telegram_user_id', userId)
...
// ============ دليل كشف الحساب: كل الأشخاص، النشط والمتصفّي ============
// التصفية تغيّر الرصيد النشط فقط؛ لا نحذف أي عملية من جدول debts.
export async function getDebtPeople(userId) {
  const [{ data: debts, error: debtsError }, { data: settlements, error: settlementsError }] = await Promise.all([
    supabase.from('debts').select('person_name, amount, currency_code, direction, is_repayment, created_at').eq('telegram_user_id', userId).order('created_at', { ascending: false }),
    supabase.from('debt_settlements').select('person_name, settled_at').eq('telegram_user_id', userId).order('settled_at', { ascending: false }),
  ]);
  if (debtsError) console.error('getDebtPeople debts select error:', JSON.stringify(debtsError));
  if (settlementsError) console.error('getDebtPeople settlements select error:', JSON.stringify(settlementsError));

  const lastSettlementByPerson = {};
  for (const settlement of settlements || []) {
    const key = String(settlement.person_name || '').trim().toLowerCase();
    if (!key) continue;
    if (!lastSettlementByPerson[key] || settlement.settled_at > lastSettlementByPerson[key]) lastSettlementByPerson[key] = settlement.settled_at;
  }

  const byPerson = {};
  for (const debt of debts || []) {
    const key = String(debt.person_name || '').trim().toLowerCase();
    if (!key) continue;
    if (!byPerson[key]) byPerson[key] = { name: debt.person_name, activeNet: 0, transactionCount: 0, lastActivity: debt.created_at, lastSettlement: lastSettlementByPerson[key] || null };
    const entry = byPerson[key];
    const signed = describeDebtLine(debt).signed;
    entry.transactionCount += 1;
    if (!entry.lastActivity || debt.created_at > entry.lastActivity) entry.lastActivity = debt.created_at;
    if (!entry.lastSettlement || debt.created_at > entry.lastSettlement) entry.activeNet += signed;
  }

  return Object.values(byPerson)
    .map((person) => ({ ...person, status: person.activeNet === 0 ? 'settled' : 'active' }))

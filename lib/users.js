...
export async function getTrialStartedAt(userId) {
  const { data, error } = await supabase
    ...
    .eq('telegram_user_id', userId)
    .maybeSingle();
  // مهم: لو حصل خطأ هنا (زي عمود ناقص في قاعدة البيانات) لازم نعرف بسرعة، لأن الصمت هنا
  // كان قبل كده بيخلي isInTrial() ترجع true للأبد من غير ما حد ينتبه (شوف sql/schema.sql).
  if (error) console.error('getTrialStartedAt error:', JSON.stringify(error));

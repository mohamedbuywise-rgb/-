...
// اتقسمت لقلب مشترك (extractReceiptFromBase64) بيستخدمه مسار تليجرام (بيوصله file_id فيحمّل الصورة الأول)
// ومسار الداشبورد (Smart Receipt Scanner) اللي بيوصله base64 جاهز من رفع مباشر في المتصفح — نفس الـ prompt
// بالظبط في الحالتين عشان دقة القراءة تفضل واحدة.
async function extractReceiptFromBase64(base64Image, mimeType) {
  try {
...
- category اختار أقرب فئة منطقية من القائمة بناءً على اسم المحل أو نوع المشتريات (سوبر ماركت/بقالة → تسوق، مطعم/كافيه → أكل، صيدلية → صحة، إلخ).`;
    ...
    console.error('extractReceiptFromBase64 failed:', err);
    return null;
  }
}
...
export async function extractReceiptFromImage(fileId) {
  try {
    ...
    return await extractReceiptFromBase64(base64Image, mimeType);
  } catch (err) {
...
export async function extractReceiptFromImageBase64(base64Image, mimeType) {
  return extractReceiptFromBase64(base64Image, mimeType || 'image/jpeg');
}

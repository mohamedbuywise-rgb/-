...
// ============ تسجيل مصروف في قاعدة البيانات بس (من غير حساب إجمالي ولا إرسال تليجرام) ============
// مستخدمة لما يكون فيه أكتر من عملية في نفس التسجيل الصوتي (زي "٥٠ غدا و١٠٠ تاكسي")،
// عشان منحسبش إجمالي اليوم بعد كل عملية على حدة، نحسبه مرة واحدة في الآخر بدل كده.
export async function insertExpense(expense, text, userId) {
...
// ============ إجمالي مصروفات النهاردة بس (من غير تسجيل حاجة جديدة) ============
export async function getTodayExpensesTotal(userId) {
...
// لما يكون التسجيل عبارة عن مصروف واحد بس)
export async function insertExpenseAndGetTodayTotal(expense, text, userId) {
  await insertExpense(expense, text, userId);
  return getTodayExpensesTotal(userId);
}

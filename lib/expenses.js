...
import { maybeSendBudgetAlert, maybeSendLargeExpenseAlert } from './webPush.js';
...
export async function recordExpense(expense, text, userId, chatId, extraFooter = '', sourceMeta = {}) {
  ...
  const sourceFields = {
    ...
    source_account_last4: String(sourceMeta.account_last4 || '').slice(0, 8) || null,
  };
  ...
  if (error && ['PGRST204', '42703'].includes(error.code)) {
    // عمود source_account_last4 لسه مش موجود: نحفظ باقي بيانات المصدر (البنك) من غيره قبل ما نرجع للأساسي
    const { source_account_last4: _omit, ...withoutAccount } = sourceFields;
    ({ data: inserted, error } = await supabase.from('expenses').insert({ ...baseExpense, ...withoutAccount }).select('id').single());
    if (error && ['PGRST204', '42703'].includes(error.code)) {
  ...
  // تنبيه لو المصروف ده أكبر بكتير من المعتاد (بيخرج فورًا لو المبلغ صغير، فمفيش تكلفة على أغلب المصاريف)
  await maybeSendLargeExpenseAlert(userId, { id: inserted.id, amount: Number(expense.amount), category, description: expense.note || '' }).catch((error) => console.error('large expense push alert failed:', error));
}
...
async function getExpensesBetween(userId, start, end) {
  let { data, error } = await supabase
    .from('expenses')
    .select('id, amount, currency_code, category, description, created_at, source, source_bank_key, source_bank_label, source_account_last4')
    .eq('telegram_user_id', userId)
  ...
  if (error && ['PGRST204', '42703'].includes(error.code)) {
    ({ data, error } = await supabase
      .from('expenses')
      .select('id, amount, currency_code, category, description, created_at, source, source_bank_key, source_bank_label')
      .eq('telegram_user_id', userId)
      .gte('created_at', start.toISOString())
      .lt('created_at', end.toISOString())
      .order('created_at', { ascending: false }));
  ...
  // قاعدة بيانات قديمة لسه ما فيهاش أعمدة source_bank_*: نرجع للأعمدة الأساسية بدون ما نكسر الشاشة
  if (error && ['PGRST204', '42703'].includes(error.code)) {
    ({ data, error } = await supabase
      .from('expenses')
...
export function normalizeSubscriptionKey(description = '') {
...
// ============ صياد الاشتراكات 2.0 — بيفرّق بين اشتراك ثابت (قيمة شبه ثابتة كل شهر) ومشترى بيتكرر (قيمته بتفرق زي البقالة) ============
export async function detectRecurringSubscriptions(userId, monthsBack = 4) {
  const since = new Date();
  since.setMonth(since.getMonth() - monthsBack, 1);
  since.setHours(0, 0, 0, 0);

  const [{ data, error }, { data: flagRows, error: flagError }] = await Promise.all([
    supabase
      .from('expenses')
      .select('id, amount, currency_code, category, description, created_at')
      .eq('telegram_user_id', userId)
    ...
    supabase.from('subscription_flags').select('subscription_key').eq('telegram_user_id', userId),
  ]);
  if (error) { console.error('detectRecurringSubscriptions select error:', JSON.stringify(error)); return []; }
  if (flagError) console.error('detectRecurringSubscriptions flags error:', JSON.stringify(flagError));
  const flaggedKeys = new Set((flagRows || []).map((r) => r.subscription_key));
  ...
  for (const key of Object.keys(byKey)) {
    ...
    // ============ اشتراك ثابت = القيمة شبه واحدة كل شهر (فرق أقل من 5%). غير كده = مشترى بيتكرر (بقالة، تاكسي...) ============
    const type = Math.abs(priceChangePercent) <= 5 ? 'subscription' : 'purchase';
    results.push({
      key,
      ...
      priceChangePercent,
      increaseAmount: Math.max(0, lastAmount - prevAmount),
      lastChargedAt: byKey[key].months[lastMonth].createdAt,
      type,
      flagged: flaggedKeys.has(key),
    });
  }
...
// ============ تعليم/إلغاء تعليم اشتراك ثابت كـ"مش بستخدمه" — بيتزامن بين الأجهزة عشان مخزّن في الداتابيز ============
export async function flagSubscriptionUnused(userId, key, label, amount) {
  if (!key) return { error: 'مفيش اسم اشتراك.' };
  const { error } = await supabase
    .from('subscription_flags')
    .upsert(
      { telegram_user_id: userId, subscription_key: key, label: label || key, last_amount: Number(amount) || 0, flagged_at: new Date().toISOString() },
      { onConflict: 'telegram_user_id,subscription_key' }
    );
  if (error) { console.error('flagSubscriptionUnused error:', JSON.stringify(error)); return { error: 'حصل خطأ، جرب تاني.' }; }
  return { ok: true };
}
export async function unflagSubscriptionUnused(userId, key) {
  if (!key) return { error: 'مفيش اسم اشتراك.' };
  const { error } = await supabase.from('subscription_flags').delete().eq('telegram_user_id', userId).eq('subscription_key', key);
  if (error) { console.error('unflagSubscriptionUnused error:', JSON.stringify(error)); return { error: 'حصل خطأ، جرب تاني.' }; }
  return { ok: true };
}

// ============ كارت "فرص التوفير" — بيجمع 3 مصادر: اشتراكات معلّمة كمش مستخدمة + اشتراكات غلت فجأة + تقدير من الفئات الكمالية ============
const DISCRETIONARY_CATEGORIES = ['تسوق', 'ترفيه', 'اشتراكات', 'هدايا وتبرعات', 'شخصي وعناية'];
export async function getSavingsOpportunities(userId) {
  const subs = await detectRecurringSubscriptions(userId);
  const subscriptionsOnly = subs.filter((s) => s.type === 'subscription');
  const unusedTotal = subscriptionsOnly.filter((s) => s.flagged).reduce((sum, s) => sum + s.amount, 0);
  const priceIncreaseTotal = subscriptionsOnly.filter((s) => s.priceChangePercent > 5).reduce((sum, s) => sum + s.increaseAmount, 0);
  ...
  const { data: monthExpenses, error } = await supabase
    ...
    .gte('created_at', start.toISOString())
    .lt('created_at', end.toISOString());
  if (error) console.error('getSavingsOpportunities month expenses error:', JSON.stringify(error));
  const discretionaryTotal = (monthExpenses || [])
    .filter((e) => DISCRETIONARY_CATEGORIES.includes(e.category))
    .reduce((sum, e) => sum + Number(e.amount || 0), 0);
  const discretionaryEstimate = Math.round((discretionaryTotal * 0.2) / 10) * 10;

  const total = Math.round(unusedTotal + priceIncreaseTotal + discretionaryEstimate);
  return {
    total,
    breakdown: {
      unusedSubscriptions: Math.round(unusedTotal),
      priceIncreases: Math.round(priceIncreaseTotal),
      discretionaryEstimate,
    },
    subscriptions: subscriptionsOnly,
    recurringPurchases: subs.filter((s) => s.type === 'purchase'),
  };
}

...
import { audioExtension, preprocessAudioBuffer } from './audioPreprocess.js';
...
export async function transcribeVoice(fileId) {
  ...
  // 3) نقلل الدوشة ونوحّد الملف قبل Whisper، مع fallback آمن للصوت الخام
  const prepared = await preprocessAudioBuffer(Buffer.from(audioBuffer), { mimeType: 'audio/ogg' });
  console.log('TELEGRAM_GROQ_VOICE_DENOISED:', prepared.denoised, 'inputBytes:', audioBuffer.byteLength, 'outputBytes:', prepared.buffer.length);
  const result = await callGroqWhisper(new Blob([prepared.buffer], { type: prepared.denoised ? 'audio/wav' : 'audio/ogg' }), prepared.denoised ? 'voice.wav' : 'voice.ogg');
...
أنواع المعاملات:
1. "expense" (مصروف استهلاكي عادي — يشمل أيضًا شراء أي حاجة محددة بالاسم زي "اشتريت بروتين بألفين" أو "اشتريت هدوم بـ500"، ده كله expense عادي برضو مهما كان نوع الكلام المستخدم: شراء/دفعت/صرفت كلهم نفس المعنى):
   {"type": "expense", "amount": رقم, "currency_code": "ISO 4217 مثل EGP/USD/EUR", "category": "واحدة من: ${CATEGORIES.join(', ')}", "note": "وصف حقيقي أو الفئة نفسها لو لم توجد تفاصيل", "item": "اسم الشيء المشترى لو مذكور وإلا فاضي", "raw_text": "العبارة الأصلية"}
...
4. "asset" (شراء أصل/جهاز/معدات مرتفعة القيمة **للاستخدام الشخصي**، زي موبايل أو لابتوب أو معدة عمل — **ده نوعه أصل للتصنيف بس بيتحسب مصروف عادي في كل الشاشات**):
...
"اشتريت بروتين بألفين" -> {"transactions": [{"type":"expense","amount":2000,"category":"صحة","item":"بروتين","note":"شراء بروتين","raw_text":"اشتريت بروتين بألفين"}]}
  ...
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        temperature: 0,
        max_tokens: 4000,
        // gpt-oss موديل بيفكّر قبل الرد — تفكير منخفض = أسرع بكتير للتصنيف البسيط
        ...(/gpt-oss/i.test(GROQ_TEXT_MODEL) ? { reasoning_effort: 'low' } : {}),
      }),
...
// ============ المسار السريع للفويس: تفريغ + تصنيف في نداء واحد عبر Gemini (audio understanding) ============
// بدل ما نعمل استدعاءين متتاليين (Whisper للتفريغ ثم استدعاء تاني لـ classifyMessage للتصنيف)،
// هنا بنبعت الصوت مباشرة لـ Gemini مع نفس قواعد التصنيف بالظبط، وهو بيرجع النص المفرّغ + المعاملات
// المصنّفة في رد واحد. ده بيقلل زمن الاستجابة (نداء واحد بدل اتنين) وبيحسّن الدقة لأن الموديل
// بيفهم كلمات زي "مواصلات" من السياق (جنب رقم فلوس) مباشرة، مش بيمر بخطوة تفريغ نصي وسيطة ممكن
// يغلط فيها (Whisper أحيانًا بيسمعها "مصلات" مثلاً).
// ⚠️ قواعد التصنيف هنا نسخة مطابقة لنفس القواعد في classifyMessage فوق — لو عدّلت حاجة هناك
// (فئة جديدة، قاعدة دين جديدة...) لازم تعدّلها هنا كمان عشان الاتنين يفضلوا متطابقين.
// بيرجع null لو فشل (مفيش مفتاح Gemini / رد فاضي / JSON مش سليم) عشان الكولر يرجع للمسار القديم.
async function classifyVoiceAudioViaGemini(base64Audio, mimeType) {
  if (!GEMINI_API_KEY) return null;
  const prompt = `أنت مساعد مالي بيسمع تسجيل صوتي بالعامية المصرية (ممكن يكون فيه ضوضاء خلفية أو نطق سريع أو كلمات متلخبطة صوتيًا).
افهم الكلام كويس مع الانتباه إن كلمات زي "مواصلات" ممكن تتنطق بسرعة فتشبه كلمة تانية صوتيًا — استخدم السياق
(زي وجود رقم فلوس جنبها) عشان تفهم الكلمة الصح، مش بس شكل الصوت الحرفي.

المطلوب منك حاجتين:
1. تفرّغ الكلام بالظبط زي ما اتقال.
2. تحدد نوع كل معاملة مالية مذكورة وتستخرج بياناتها، حسب القواعد دي بالظبط:

أنواع المعاملات:
1. "expense" (مصروف استهلاكي عادي — يشمل أيضًا شراء أي حاجة محددة بالاسم، شراء/دفعت/صرفت كلهم نفس المعنى):
   {"type": "expense", "amount": رقم, "currency_code": "ISO 4217 مثل EGP/USD/EUR", "category": "واحدة من: ${CATEGORIES.join(', ')}", "note": "وصف حقيقي أو الفئة نفسها لو لم توجد تفاصيل", "item": "اسم الشيء المشترى لو مذكور وإلا فاضي", "raw_text": "العبارة الأصلية"}
2. "income" (دخل/بيع/تحصيل أموال):
   {"type": "income", "amount": رقم, "currency_code": "ISO 4217", "category": "دخل/بيع خدمة/راتب/هدايا أو أقرب وصف", "note": "ماذا باع أو حصل", "raw_text": "العبارة الأصلية"}
4. "asset" (شراء أصل/جهاز/معدات مرتفعة القيمة **للاستخدام الشخصي**، زي موبايل أو لابتوب — **ده نوعه أصل للتصنيف بس بيتحسب مصروف عادي في كل الشاشات**):
   {"type": "asset", "amount": رقم, "currency_code": "ISO 4217", "category": "أجهزة/معدات أو أقرب فئة", "item": "اسم الأصل أو الموديل", "note": "وصف الأصل", "raw_text": "العبارة الأصلية"}
11. "portfolio_buy" (شراء أصل **استثماري** يُحتفظ به كمخزن قيمة — ذهب/عملة رقمية/أسهم/صناديق. **مش مصروف ولا دخل**):
   {"type": "portfolio_buy", "amount": رقم (المبلغ المدفوع), "currency_code": "ISO 4217", "asset_name": "اسم الأصل زي ما اتقال", "quantity": رقم الكمية لو مذكورة وإلا فاضي, "unit": "وحدة الكمية لو مذكورة وإلا فاضي", "note": "وصف قصير", "raw_text": "العبارة الأصلية"}
12. "portfolio_sell" (بيع أصل استثماري موجود عند المستخدم. **دخل حقيقي**):
   {"type": "portfolio_sell", "amount": رقم (المبلغ المقبوض), "currency_code": "ISO 4217", "asset_name": "اسم الأصل المباع", "quantity": رقم الكمية المباعة لو مذكورة وإلا فاضي, "unit": "وحدة الكمية لو مذكورة وإلا فاضي", "note": "وصف قصير", "raw_text": "العبارة الأصلية"}
5. "transfer" (تحويل بين حسابات المستخدم، أو لشخص/من شخص بدون سياق تجاري واضح):
   {"type": "transfer", "amount": رقم, "currency_code": "ISO 4217", "note": "مصدر التحويل ووجهته", "counterparty": "اسم الشخص أو رقمه لو موجود وإلا فاضي", "needs_review": true أو false, "raw_text": "العبارة الأصلية"}
6. "withdrawal" (سحب كاش من حساب/فيزا/محفظة لـ ATM — نقل مكان الفلوس بس، مش إنفاق):
   {"type": "withdrawal", "amount": رقم, "currency_code": "ISO 4217", "note": "مصدر السحب", "raw_text": "العبارة الأصلية"}
7. "deposit" (إيداع، ومش واضح إنه دخل فعلي):
   {"type": "deposit", "amount": رقم, "currency_code": "ISO 4217", "note": "مصدر الإيداع", "raw_text": "العبارة الأصلية"}
8. "refund" (استرداد مبلغ):
   {"type": "refund", "amount": رقم, "currency_code": "ISO 4217", "category": "الفئة المناسبة", "note": "ما تم استرداده", "raw_text": "العبارة الأصلية"}
9. "debt" (دين/سلفة/مرتجع):
...
قواعد مهمة:
- لو الرسالة فيها اسم تاجر/خدمة معروفة = "expense" مباشرة. "سحب"/"Withdrawal" = withdrawal دايمًا. "إيداع"/"Deposit" من غير دليل دخل = deposit.
- تحويل بين حسابات المستخدم نفسه = transfer بـ needs_review: false. تحويل لشخص/رقم بدون سياق تجاري = transfer بـ needs_review: true وحط الاسم في counterparty.
- "asset" فقط للأجهزة اللي بتتستخدم فعليًا. "portfolio_buy/sell" للأصول الاستثمارية (ذهب/عملات رقمية/أسهم/صناديق) بس.
- الفئة المختصرة مع المبلغ تكفي: "أكل 500" = expense فئة "أكل"، "مواصلات 300" = expense فئة "مواصلات".
- لو لم تُذكر عملة فاستعمل "EGP". دولار/USD="USD"، يورو/EUR="EUR"، ريال سعودي/SAR="SAR"، درهم/AED="AED"، جنيه إسترليني/GBP="GBP".
- لو الرسالة فيها كذا معاملة (مثلاً "صرفت 50 جنيه أكل و100 مواصلات")، رجّعهم كلهم في القائمة من غير حد أقصى.
- اتجاه الدين: "واصل إلى/لـ فلان"/"سلفت فلان"/"دفعت لفلان" = lent (أنت الدائن). "واصل من فلان"/"استلفت من فلان"/"أخدت من فلان" = borrowed (عليك الفلوس).
- is_repayment = true لو فيه سداد صريح لدين قديم. لو الجملة مفيهاش رقم أو مش مفهومة، رجّع {"type": "unknown"}.
- المبالغ الكبيرة (50 الف، نص مليون) حولها لأرقام صحيحة. الأرقام المنطوقة بالعامية (مية=100، ميتين=200، تلاتمية=300... إلخ، وتركيبات زي "مية وعشرين"=120) = amount رقمي دايمًا.
- category لازم تكون واحدة بالظبط من القائمة فوق، مفيش فئة "أخرى" — اختار أقرب فئة منطقية دايمًا.
- "note" لازم تتملي بتفصيلة حقيقية قصيرة (2-4 كلمات) من كلام المستخدم لو موجودة، وإلا سيبها فاضية.
- raw_text يحتوي العبارة الأصلية كاملة من التفريغ.

رجّع JSON فيه مفتاحين بس، من غير أي شرح أو علامات markdown:
{"transcript": "النص الكامل المفرّغ من الصوت زي ما اتقال بالظبط بالعامية المصرية", "transactions": [ ...قائمة المعاملات بنفس التنسيق فوق، ممكن تكون فاضية لو مفيش معاملة مالية واضحة... ]}`;
  ...
  try {
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(GEMINI_TEXT_MODEL)}:generateContent`,
      {
        ...
        body: JSON.stringify({
          contents: [
            { role: 'user', parts: [{ text: prompt }, { inline_data: { mime_type: mimeType, data: base64Audio } }] },
          ],
    ...
    const data = await res.json().catch(() => ({}));
    console.log('GEMINI_VOICE_COMBINED_STATUS:', res.status);
    if (!res.ok) {
      console.error('classifyVoiceAudioViaGemini HTTP error:', res.status, JSON.stringify(data));
      return null;
    }
    const reply = (data.candidates?.[0]?.content?.parts || []).map((p) => p.text || '').join('').trim();
    if (!reply) {
      console.error('classifyVoiceAudioViaGemini empty/blocked response:', JSON.stringify(data));
      return null;
    }
    const cleaned = reply.replace(/^```json\s*|^```\s*|```\s*$/g, '').trim();
    ...
    console.error('classifyVoiceAudioViaGemini failed:', err);
    return null;
  }
}

// ============ تفريغ + تصنيف الفويس نوت (تليجرام) في نداء واحد — المسار السريع الجديد ============
// بيجرب Gemini المدمج الأول (خطوة واحدة). لو فشل، بيرجع تلقائيًا للمسار القديم بالظبط
// (denoise -> Whisper -> فشل؟ -> Gemini تفريغ نص بس -> classifyMessage) عشان الموثوقية تفضل زي الأول.
export async function transcribeAndClassifyVoice(fileId) {
  const fileInfoRes = await fetch(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/getFile?file_id=${fileId}`);
  const fileInfo = await fileInfoRes.json();
  const filePath = fileInfo.result.file_path;
  const fileUrl = `https://api.telegram.org/file/bot${TELEGRAM_TOKEN}/${filePath}`;
  ...
  const combined = await classifyVoiceAudioViaGemini(base64Audio, 'audio/ogg');
  if (combined) return combined;

  console.error('TELEGRAM_VOICE_COMBINED_FAILED, falling back to Whisper+classify chain');
  const prepared = await preprocessAudioBuffer(Buffer.from(audioBuffer), { mimeType: 'audio/ogg' });
  const whisperResult = await callGroqWhisper(
    new Blob([prepared.buffer], { type: prepared.denoised ? 'audio/wav' : 'audio/ogg' }),
    prepared.denoised ? 'voice.wav' : 'voice.ogg',
  );
  const transcript = whisperResult.ok ? whisperResult.text : await askGeminiTranscribe(base64Audio, 'audio/ogg');
  if (!transcript) return { transcript: '', transactions: [] };
  const transactions = await classifyMessage(transcript);
  return { transcript, transactions };
}

// ============ تفريغ + تصنيف الفويس نوت (الداشبورد) في نداء واحد — نفس منطق تليجرام، بس بياخد base64 جاهز ============
export async function transcribeAndClassifyVoiceBase64(audioBase64, mimeType = 'audio/webm') {
  const clean = String(audioBase64 || '').replace(/^data:[^,]+,/, '');
  if (!clean) return { success: false, error: 'ملف الصوت فاضي.', transcript: '', transactions: [] };

  const combined = await classifyVoiceAudioViaGemini(clean, mimeType);
  if (combined) return { success: true, ...combined };

  console.error('DASHBOARD_VOICE_COMBINED_FAILED, falling back to Whisper+classify chain');
  const transcribed = await transcribeAudioBase64(audioBase64, mimeType);
  if (!transcribed.success) return { success: false, error: transcribed.error, transcript: '', transactions: [] };
  const transactions = await classifyMessage(transcribed.text);
  return { success: true, transcript: transcribed.text, transactions };
}
    ...
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(GEMINI_TEXT_MODEL)}:generateContent`,
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            temperature,
            // موديلات Gemini بتحسب "التفكير" من نفس الحد؛ لو الحد صغير الرد بيرجع فاضي، فبنوسّعه.
...
async function askGroqText(prompt, { temperature = 0.4, maxTokens = 300 } = {}) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      // سبب "مفيش رد": gpt-oss موديل بيفكّر، والتفكير بيتحسب من نفس max_tokens؛ مع حد صغير (260) كل الحد كان بيروح في التفكير
      // والرد الفعلي بيرجع فاضي. فبنوسّع الحد ونخلّي التفكير خفيف ومخفي (في المحاولة التانية من غير الإضافات احتياطًا لو Groq رفضها).
      const extras = isReasoningModel && attempt === 1 ? { reasoning_effort: 'low', include_reasoning: false } : {};
      const tokenBudget = isReasoningModel ? Math.max(maxTokens * 5, 1500) : maxTokens;
      const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${GROQ_API_KEY}`,
      ...
      if (!res.ok) {
        ...
        console.error('askGroqText empty response, attempt:', attempt, 'finish_reason:', data.choices?.[0]?.finish_reason);
      }
    } catch (err) {
...
export async function transcribeAudioBase64(audioBase64, mimeType = 'audio/webm') {
  ...
  try {
    ...
    const prepared = await preprocessAudioBuffer(bytes, { mimeType });
    console.log('DASHBOARD_GROQ_VOICE_DENOISED:', prepared.denoised, 'inputBytes:', bytes.length, 'outputBytes:', prepared.buffer.length);
    // امتداد الملف لازم يتطابق مع نوعه الحقيقي؛ بعد المعالجة يكون WAV موحدًا.
    const ext = prepared.denoised ? 'wav' : audioExtension(mimeType);
    const preparedMime = prepared.denoised ? 'audio/wav' : mimeType;
    const result = await callGroqWhisper(new Blob([prepared.buffer], { type: preparedMime }), `dabbar-voice.${ext}`);
    console.log('DASHBOARD_GROQ_VOICE result:', result.ok, 'model:', result.modelUsed, 'bytes:', prepared.buffer.length, 'mime:', preparedMime);
    ...
    const geminiText = await askGeminiTranscribe(prepared.buffer.toString('base64'), preparedMime);
...
// ============ "هزقني بدم خفيف" — قرص كوميدي على مصاريف المستخدم، مبني على أرقامه الحقيقية بس ============
// نفس بيانات "اسأل دبّر" (dataContext)، لكن برومبت مختلف تمامًا في الأسلوب: هزار مصري خفيف الدم
// وواضح فيه تفاصيل (اتصرف كام وفين)، من غير أي إهانة أو تجريح حقيقي — الهدف يضحك المستخدم مش يزعله.
export async function askDabbarRoast(dataContext) {
  const prompt = `إنت "دبّر"، مساعد مالي مصري خفيف الدم بيهزر مع المستخدم على مصاريفه بدم خفيف وضحك، من غير أي إهانة أو تحقير حقيقي.
دي بيانات المستخدم الحقيقية (مصاريفه، ديونه، وهدفه المالي لو عنده):
${dataContext}

المطلوب: "هزقه بدم خفيف" على أعلى حاجة بيصرف فيها أو أي عادة صرف لافتة في البيانات دي.
اتكلم بالعامية المصرية بأسلوب ساخر كوميدي زي صاحبه اللي بيقرصه على القهوة (مثلاً: "يا عم اللي انت عملته في نفسك ده"، "بالصلاة ع النبي البودجيت طار في...")،
ووضح بالتفصيل هو صرف كام وفي إيه بالظبط بأسلوب مضحك، من غير ما تخترع أرقام مش موجودة فوق.
خلي الرد 3-5 جمل بحد أقصى، وابدأ فيه بإيموجي مناسب، واختم بخفة دم أو نصيحة هزلية بسيطة — من غير ما تحوّل الآخر لموعظة جدية أو تخوّفه.`;

  const reply = await askGroqText(prompt, { temperature: 0.8, maxTokens: 260 });
  return reply || 'معلش يا نجم، هزارتي راحت في الزحمة 😅 جرب تاني كمان شوية.';
}
...
// ============ تحليل رسالة SMS بنك/محفظة (عربي/إنجليزي) — برومبت مخصوص ============
// بيتنده بس لما المحلل القاعدي (lib/smsParser.js) مش واثق. بياخد النص بعد ما اتشال منه رقم الرصيد/الرسوم
// (اتحطوا كـ [BALANCE]/[FEE]) عشان الموديل ما يلخبطش بينهم وبين مبلغ العملية، ومعاه تلميحات من المحلل القاعدي.
export async function classifyBankSms(text, hints = {}) {
  const hintLines = [];
  if (hints.amount) hintLines.push(`- مبلغ محتمل للعملية: ${hints.amount} ${hints.currency || ''}`.trim());
  if (hints.direction) hintLines.push(`- الاتجاه المحتمل: ${hints.direction === 'in' ? 'داخل للحساب' : 'خارج من الحساب'}`);
  if (hints.merchant) hintLines.push(`- الجهة/التاجر المحتمل: ${hints.merchant}`);
  if (hints.counterparty) hintLines.push(`- الطرف الآخر المحتمل (شخص/رقم): ${hints.counterparty}`);

  const prompt = `أنت محلل رسائل SMS للبنوك والمحافظ الإلكترونية في مصر (فودافون كاش، إنستاباي، أورنج كاش، اتصالات كاش، WE Pay، CIB، الأهلي، مصر، QNB...). الرسالة بالعربي أو الإنجليزي أو مخلوطة.
مهمتك: استخرج العملية المالية المكتملة من الرسالة وارجع JSON فقط بالشكل: {"transactions":[ ... ]} (عادةً عنصر واحد).

قواعد المبلغ (أهم قاعدة):
- amount = مبلغ العملية نفسها فقط، رقم موجب من غير فواصل آلاف.
- ممنوع تستخدم كمبلغ: الرصيد (Balance/الرصيد/رصيدك/[BALANCE])، الرسوم ([FEE]/رسوم/Fee)، الحد، رقم البطاقة/الحساب/العملية/المرجع، رقم الموبايل، التاريخ والوقت.
- لو فيه رسوم اكتبها في fee (رقم) ومتخليهاش هي المبلغ.
- الأرقام ممكن تكون بالهندي (٢٥٠) — حوّلها لأرقام عادية.

الأنواع (type):
- "expense": شراء أو دفع لتاجر/خدمة/فاتورة/شحن رصيد. حط category واحدة بالظبط من: ${CATEGORIES.join('، ')}، وحط item = اسم التاجر لو موجود.
- "income": دخل واضح (مرتب/راتب/إيراد/كاش باك).
- "deposit": إيداع/إضافة للحساب مش واضح إنه دخل.
- "withdrawal": سحب نقدي (ATM أو وكيل أو Cash out).
- "transfer": تحويل لشخص/رقم/حساب أو استلام تحويل (InstaPay/محفظة/تحويل بنكي). لازم direction = "in" لو الفلوس داخلة لصاحب الرسالة أو "out" لو خارجة منه، و counterparty = اسم الشخص أو رقمه، و needs_review = true لو الطرف شخص أو رقم موبايل.
- "refund": استرداد/رد مبلغ.
- "unknown": مش عملية مكتملة (OTP، رفض، عرض ترويجي، استعلام رصيد، إعلان).

حقول كل عنصر: type, amount, currency_code (ISO مثل EGP/USD، الافتراضي EGP), direction (للتحويل فقط), category (للمصروف/الاسترداد), note (وصف قصير 2-5 كلمات بالعربي), item, counterparty, needs_review, fee, balance, raw_text (الرسالة كما هي).

أمثلة:
"تم تحويل 300.00 جنيه من محفظتك إلى 01012345678 بنجاح. رسوم الخدمة [FEE] جنيه. رصيدك الحالي [BALANCE] جنيه" -> {"transactions":[{"type":"transfer","amount":300,"currency_code":"EGP","direction":"out","counterparty":"01012345678","needs_review":true,"note":"تحويل صادر من فودافون كاش"}]}
"You received EGP 500.00 from Ahmed Mohamed via InstaPay" -> {"transactions":[{"type":"transfer","amount":500,"currency_code":"EGP","direction":"in","counterparty":"Ahmed Mohamed","needs_review":true,"note":"تحويل وارد عبر انستاباي"}]}
"Your card ending 1234 was charged EGP 250.00 at CARREFOUR. Available balance [BALANCE]" -> {"transactions":[{"type":"expense","amount":250,"currency_code":"EGP","category":"تسوق","item":"CARREFOUR","note":"CARREFOUR"}]}
"تم إيداع مبلغ 5000 جنيه في حسابكم" -> {"transactions":[{"type":"deposit","amount":5000,"currency_code":"EGP","note":"إيداع في الحساب"}]}
"ATM withdrawal EGP 2000" -> {"transactions":[{"type":"withdrawal","amount":2000,"currency_code":"EGP","note":"سحب من ATM"}]}
"رمز التحقق 123456 لا تشاركه" -> {"transactions":[{"type":"unknown"}]}
${hintLines.length ? `\nمعلومات مستخرجة آليًا (قد تكون ناقصة أو غلط — تحقق منها من النص ولا تعتمد عليها لو الرسالة بتقول غير كده):\n${hintLines.join('\n')}\n` : ''}
الرسالة: """${String(text).slice(0, 1500)}"""`;
  ...
  try {
    ...
    const data = await res.json().catch(() => ({}));
    console.log('GROQ_BANK_SMS_STATUS:', res.status);
    if (res.ok) rawText = data.choices?.[0]?.message?.content || '';
    else console.error('GROQ_BANK_SMS failed, will try Gemini fallback:', res.status);
  } catch (err) {
    console.error('GROQ_BANK_SMS request failed, will try Gemini fallback:', err);
  }
  if (!rawText) {
    const geminiRaw = await askGeminiText(prompt, { temperature: 0, maxTokens: 1200 });
    rawText = geminiRaw.replace(/^```json\s*|^```\s*|```\s*$/g, '').trim();
  }
  if (!rawText) return [{ type: 'unknown' }];
  try {

...
// ⚠️ رجّعنا من openai/gpt-oss-20b لـ llama-3.3-70b-versatile: الموديل الصغير كان بيغلط في ربط
// كل رقم بالبند الصح لما الرسالة فيها 5+ معاملات مع بعض (بيكرر نفس الرقم لعناصر مختلفة، أو
// بيدمج بندين في واحد). التكلفة أعلى شوية لكن الدقة مع رسايل الفويس الطويلة بتفرق فرق كبير.
// لو غيّرته تاني افحصه كويس برسالة فيها 6-7 بنود مختلطة (مصاريف + دين) قبل ما تنشره.
// موديل رخيص للرسايل البسيطة (رقم واحد أو اتنين بس) — أغلب رسايل المستخدمين كده فعلاً
export const GROQ_TEXT_MODEL = process.env.GROQ_TEXT_MODEL || 'openai/gpt-oss-20b';
// موديل أقوى (وأغلى) بيتستخدم بس لما الرسالة فيها 3+ أرقام مع بعض، عشان يقلل تكلفة أغلب
// الرسايل البسيطة ويستخدم الدقة العالية بس فين محتاجينها فعلًا.
export const GROQ_TEXT_MODEL_COMPLEX = process.env.GROQ_TEXT_MODEL_COMPLEX || 'llama-3.3-70b-versatile';
// أقل عدد أرقام في الرسالة عشان نعتبرها "معقدة" ونستخدم الموديل الأغلى من الأول (مش بعد فشل)
export const COMPLEX_MESSAGE_NUMBER_THRESHOLD = Number(process.env.COMPLEX_MESSAGE_NUMBER_THRESHOLD || 3);
...
// Web Push: المفتاح العام يخرج للمتصفح، والخاص يظل في بيئة الخادم فقط.
export const VAPID_PUBLIC_KEY = process.env.VAPID_PUBLIC_KEY;
export const VAPID_PRIVATE_KEY = process.env.VAPID_PRIVATE_KEY;
export const VAPID_SUBJECT = process.env.VAPID_SUBJECT || 'mailto:admin@dabbar.app';
...
export const USAGE_LIMITS = {
  paid: { voice: 250, ocr: 25, chat: 180 },
  trial: { voice: 60, ocr: 15, chat: 50 },
};
...
// اتقلل من 30 لـ15 ثانية بعد مراجعة هامش الربح لكل مشترك (80 جنيه/شهر) — 15 ثانية كافية جدًا
// لجملة مصروفات طبيعية (حتى المعقدة زي "مواصلات 100 وشراء كذا 300...")، وبتنص تكلفة
// التفريغ والتصنيف تقريبًا من غير ما المستخدم يحس بفرق ملموس في الاستخدام العادي.
export const VOICE_MAX_DURATION_SECONDS = 15;

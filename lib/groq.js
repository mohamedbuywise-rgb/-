...
// ============ استدعاء Groq Whisper بموديل Turbo (أرخص بـ~65%) أولًا، وlarge-v3 القديم كبديل لو فشل ============
// (Turbo أضعف شوية من large-v3 في الدقة على اللهجات، فلو فشل أو رجّع نص فاضي، بنعتبره "فشل" ونجرب large-v3 فورًا)
...
export async function transcribeVoice(fileId) {
  ...
  // 3) نبعته لـ Groq Whisper (Turbo أولًا، large-v3 لو فشل)
  const result = await callGroqWhisper(new Blob([audioBuffer]), 'voice.ogg');
  if (result.ok) return result.text;

  console.error('TELEGRAM_GROQ_VOICE failed (both models), trying Gemini fallback:', result.status);
  // فشل Groq Whisper بالكامل -> نجرب Gemini بنفس الملف الصوتي قبل ما نرجّع فاضي
...
export async function transcribeAudioBase64(audioBase64, mimeType = 'audio/webm') {
  ...
  try {
    ...
    console.error('DASHBOARD_GROQ_VOICE failed (both models), trying Gemini fallback:', result.status);

    // فشل Groq Whisper (429 أو أي خطأ) -> نجرب Gemini بنفس الملف الصوتي قبل ما نستسلم

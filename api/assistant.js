...
import { extractItemizedReceiptFromImageBase64, askDabbarChat, parseUnifiedEntryText, transcribeAudioBase64 } from '../lib/groq.js';
import { getUserGlobalContext } from '../lib/globalContext.js';
...
async function requireLink(req, res) {
  ...
  const dataUserId = link?.telegram_user_id || await resolveDataUserId(userData.user);
  if (!dataUserId) {
    res.status(500).json({ error: 'تعذر تجهيز مساحة بيانات الحساب حاليًا.' });
    return null;
  }

  // Telegram اختياري؛ الحساب غير المرتبط يحصل على trial يبدأ عند إنشاء مساحة البيانات.
  const subscribed = await hasActiveSubscription(dataUserId);
  if (!subscribed) {
    const trial = await isInTrial(dataUserId);
    if (!trial) {
...
async function handleEntryDraft(userId, body, res) {
  const context = await getUserGlobalContext(userId);
  let text = (body.text || '').trim();
  if (body.audioBase64 && String(body.audioBase64).length > 8 * 1024 * 1024) {
    return res.status(413).json({ error: 'التسجيل طويل أوي. سجّل بحد أقصى 30 ثانية.' });
  }
  if (body.audioBase64) {
  ...
  const parsed = await parseUnifiedEntryText(text, context);
  if (!parsed.success) return res.status(422).json({ error: parsed.error });
  return res.status(200).json({ draft: parsed.draft, transcript: text, globalContext: context });
}

async function handleEntryConfirm(userId, body, res) {
  const draft = body.draft || {};
  const amount = Number(draft.amount);
  if (!['expense', 'debt', 'invoice'].includes(draft.type) || !Number.isFinite(amount) || amount <= 0 || amount > 100000000) {
    return res.status(400).json({ error: 'المسودة غير صالحة أو المبلغ غير صحيح.' });
  }
  const context = await getUserGlobalContext(userId);
  if (draft.type === 'expense') {
    const { data, error } = await supabase.from('expenses').insert({
      ...
      amount,
      category: String(draft.category || 'مصروف عام').slice(0, 80),
      description: String(draft.note || draft.sourceText || '').slice(0, 500),
      currency_code: context.currencyCode,
    }).select('id, amount, category, description, created_at').single();
    ...
    return res.status(200).json({ ok: true, type: 'expense', record: data, message: `تم تسجيل مصروف ${data.amount} ${context.currencyCode} في ${data.category}.` });
  }
  if (draft.type === 'debt') {
    if (!String(draft.person || '').trim()) return res.status(400).json({ error: 'اكتب اسم الشخص المرتبط بالدين قبل التأكيد.' });
    const { data, error } = await supabase.from('debts').insert({
      telegram_user_id: userId,
      person_name: String(draft.person).trim().slice(0, 120),
      amount,
      direction: draft.direction === 'lent' ? 'lent' : 'borrowed',
      note: String(draft.note || '').slice(0, 500),
      currency_code: context.currencyCode,
    }).select('id, person_name, amount, direction, created_at').single();
    if (error) { console.error('entry_confirm debt error:', JSON.stringify(error)); return res.status(500).json({ error: 'تعذر حفظ الدين.' }); }
    return res.status(200).json({ ok: true, type: 'debt', record: data, message: `تم تسجيل الدين بمبلغ ${data.amount} ${context.currencyCode}.` });
  }
  ...
  const receipt = { merchant: String(draft.merchant || '').slice(0, 160), totalAmount: amount, paymentMethod: '', invoiceNumber: '', isDebt: false, debtPerson: '', items: items.length ? items : [{ name: String(draft.note || 'فاتورة').slice(0, 160), amount, category: String(draft.category || 'مصروف عام').slice(0, 80) }] };
  const saved = await saveInvoiceRecord(receipt, userId);
  if (!saved) return res.status(500).json({ error: 'تعذر حفظ الفاتورة.' });
  return res.status(200).json({ ok: true, type: 'invoice', invoiceId: saved.invoiceId, record: receipt, message: `تم تسجيل الفاتورة بإجمالي ${amount} ${context.currencyCode}.` });
}

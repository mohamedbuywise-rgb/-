...
export async function decrementInstallment(userId, reminderId) {
  ...
  if (remaining <= 0) {
    ...
    // نحرّك تاريخ الاستحقاق لنفس اليوم الشهر الجاي (أبسط تقريب لموعد القسط الجاي)
    const next = new Date(row.due_date + 'T00:00:00');
  ...
  const { data, error } = await supabase
    .from('reminders')
    .update(patch)
    .eq('telegram_user_id', userId).eq('id', reminderId)
    .select('id, title, due_date, amount, done, installments_remaining')
    .single();

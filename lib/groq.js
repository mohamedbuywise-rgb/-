...
async function classifyViaDecomposition(text, textNumbers) {
  const phrases = await segmentIntoPhrases(text, textNumbers.length);
  // لو التقسيم نفسه فشل في إعطاء نفس عدد الأرقام، ماينفعش نكمل عليه — نرجع null عشان نرجع
  // لطريقة "الرسالة كاملة مرة واحدة" العادية كـ fallback بدل ما نبني على تقسيم غلط من الأصل.
  if (phrases.length !== textNumbers.length) {
    console.warn('SEGMENT_COUNT_MISMATCH:', phrases.length, 'expected', textNumbers.length);
    return null;
  }
...
export async function classifyMessage(text) {
  ...
  return classifyWholeMessage(text);
}

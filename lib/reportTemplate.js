...
export function buildReportHtml({ title, periodLabel, generatedAt, total, count, topCategoryName, comparisonLine, categories, flow = null }) {
  ...
  const flowRows = Object.entries(flow?.byCurrency || {})
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([currency, values]) => `
      <div class="flow-row">
        <span class="flow-currency">${esc(currencyLabel(currency))}</span>
        <span class="flow-in">+${esc(values.in)} ${esc(currencyLabel(currency))}</span>
        <span class="flow-out">−${esc(values.out)} ${esc(currencyLabel(currency))}</span>
        <strong class="flow-net">${esc(values.net >= 0 ? '+' : '−')}${esc(Math.abs(values.net))} ${esc(currencyLabel(currency))}</strong>
      </div>`)
    .join('');
  const flowHtml = flowRows ? `
      <div class="group-title">حركة السيولة (الشهر ده)</div>
      <div class="flow-card">
        <div class="flow-head"><span>العملة</span><span>داخل</span><span>خارج</span><span>الصافي</span></div>
        ${flowRows}
      </div>` : '';
...
<style>
  ...
  *{ box-sizing:border-box; margin:0; padding:0; }
  ...
  .flow-card{ background:#F8F9FB; border:1px solid #ECEDF0; border-radius:12px; padding:12px 14px; break-inside:avoid; }
  .flow-head,.flow-row{ display:grid; grid-template-columns:1fr 1.15fr 1.15fr 1.15fr; gap:8px; align-items:center; }
  .flow-head{ color:#8A8F98; font-size:9px; font-weight:700; padding-bottom:8px; border-bottom:1px solid #E8E9EC; }
  .flow-row{ color:#374151; font-size:10.5px; padding:9px 0; border-bottom:1px solid #ECEDF0; direction:rtl; }
  .flow-row:last-child{ border-bottom:none; padding-bottom:2px; }
  .flow-currency{ font-weight:800; color:#12213E; }
  .flow-in{ color:#159447; direction:ltr; unicode-bidi:isolate; }
  .flow-out{ color:#D04A55; direction:ltr; unicode-bidi:isolate; }
  .flow-net{ color:#12213E; direction:ltr; unicode-bidi:isolate; }
  .footer{

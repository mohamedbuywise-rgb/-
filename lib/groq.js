...
import { denoiseAudioBuffer } from './audioPreprocess.js';
...
export async function transcribeVoice(fileId) {
  ...
  // 2.5) تنضيف الصوت (تقليل ضوضاء + توحيد المستوى) قبل ما يوصل لـ Whisper — لو فشلت المعالجة
  // لأي سبب، بنبعت التسجيل الخام زي ما هو بدل ما نوقف الميزة كلها.
  const denoised = await denoiseAudioBuffer(Buffer.from(audioBuffer), 'ogg');
  const finalBlob = denoised ? new Blob([denoised], { type: 'audio/wav' }) : new Blob([audioBuffer]);
  const finalName = denoised ? 'voice.wav' : 'voice.ogg';

  // 3) نبعته إلى Whisper Large V3 أولًا
  const result = await callGroqWhisper(finalBlob, finalName);
...
export async function transcribeAudioBase64(audioBase64, mimeType = 'audio/webm') {
  ...
  try {
    ...
    // تنضيف الصوت (تقليل ضوضاء + توحيد المستوى) قبل الرفع لـ Whisper — نفس منطق تليجرام بالظبط.
    // لو فشلت المعالجة، بنبعت البايتس الخام زي ما هي بدل ما نوقف التسجيل كله.
    const denoised = await denoiseAudioBuffer(bytes, ext);
    const finalBlob = denoised ? new Blob([denoised], { type: 'audio/wav' }) : new Blob([bytes], { type: mimeType });
    const finalName = denoised ? 'dabbar-voice.wav' : `dabbar-voice.${ext}`;
    console.log('DASHBOARD_GROQ_VOICE_DENOISED:', Boolean(denoised));

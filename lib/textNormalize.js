...
// ============ generateDisplayLabel: عنوان قصير ونظيف للكارت، منفصل تمامًا عن "note" ============
// note ممكن تفضل فاضية أو تحتوي تفصيلة إضافية (زي اسم المطعم)، لكن الكارت محتاج عنوان مختصر
// دايمًا موجود يوضح "إيه اللي حصل" من غير ما يكرر الجملة الأصلية أو فعل الحركة. الأولوية:
// note (لو فيها تفصيلة حقيقية قصيرة) -> item -> person (للديون) -> category -> نوع المعاملة.
export function generateDisplayLabel(tx) {
  if (!tx || typeof tx !== 'object') return '';
  const note = String(tx.note || '').trim();
  const category = String(tx.category || '').trim();
  const item = String(tx.item || '').trim();
  const person = String(tx.person || '').trim();

  if (tx.type === 'debt') {
    if (person) return note ? `${person} — ${note}` : person;
    return note || category || 'دين';
  }
  if (tx.type === 'settlement') return person ? `تسوية — ${person}` : 'تسوية حساب';

  // note قصيرة وحقيقية (مش هي نفسها الفئة اسمًا) بتبقى أفضل عنوان لأنها بتحمل التفصيلة الفعلية
  if (note && note !== category) {
    return category && !containsWholePhrase(note, category) && ['expense', 'purchase', 'asset'].includes(tx.type)
      ? note
      : note;
  }
  if (item) return item;
  ...
  return ({ expense: 'مصروف', income: 'دخل', purchase: 'شراء', asset: 'أصل', transfer: 'تحويل', refund: 'استرداد' })[tx.type] || 'معاملة';
}
...
export function normalizeFinancialTransaction(transaction, sourceText = '') {
  ...
  const normalized = { ...tx, amount, currency_code, raw_text: tx.raw_text || String(sourceText || '').slice(0, 1200) };
  normalized.displayLabel = generateDisplayLabel(normalized);
  return normalized;
}
...
export function reconcileSingleTransaction(transactions, sourceText = '') {
  ...
  // ============ حماية مهمة: متطبقش الإصلاح ده إلا لو النص فعلاً فيه رقم واحد بس ============
  // الدالة دي أصلًا اتعملت عشان تصلّح رسايل بسيطة زي "أكل 500" لو الموديل رجّع نوع/فئة غلط.
  // لكن لو النص الأصلي فيه أكتر من رقم (يعني المفروض أكتر من معاملة) ووصلنا هنا بمعاملة واحدة
  // بس، فده معناه إن التصنيف فشل في تقسيم البنود (بلع باقي الأرقام) — مش إن الرسالة بسيطة.
  // في الحالة دي، extractDeterministicExpense هياخد أول رقم وأول فئة يلاقيها في النص كله
  // ويغطّي على الفشل الحقيقي بدل ما يكشفه. الأصح إننا نسيب النتيجة زي ما هي (ناقصة) عشان
  // الطبقة اللي فوق (classifyMessage) تكون خدت فرصتها في إعادة المحاولة بالمُقسّم الحتمي،
  // ومنعرضش على المستخدم كارت واحد "مصحّح" شكليًا فيه بيانات ضايعة فعليًا.
  const numbersInSource = (String(sourceText || '').match(/\d+(?:\.\d+)?/g) || []).length;
  if (numbersInSource > 1) return transactions;
...
const DEBT_OWED_TO_USER_PATTERNS = [
  /لي[\u064B-\u065F]*[اأإآيى]?\s*عند/iu,
  // ملحوظة: "ل" هنا ممكن تيجي ملزوقة في الاسم مباشرة (لأحمد) من غير مسافة، عشان كده \s* في الآخر
  // مش \s+ — لو خليناها \s+ هتفشل تلقّط "واصل لأحمد" (بتفرق عن "واصل من أحمد" اللي فيها مسافة فعلية).
  /واصل\s+(?:إلى|الى|لي|ل)(?:\s+عند)?\s*/iu,
  /عليه\s+لي/iu,
  /استلف\s+مني/iu,
  /خد(?:ت)?\s+مني/iu,
  /اخد(?:ت)?\s+مني/iu,
...
const DEBT_OWED_BY_USER_PATTERNS = [
  /علي[اىّ]?\s+(?:ل|عند)/iu,
...
// بيجيب "شريحة" من النص حوالين ذكر اسم الشخص المرتبط بالدين، بدل النص كله — عشان في رسالة
// فيها كذا دين مع كذا شخص مختلف الاتجاه، كل دين ياخد اتجاهه من الجزء اللي بيخصه بس مش من
// كلمات ممكن تكون خاصة بدين تاني في نفس الرسالة.
function localWindowAroundPerson(source, person) {
  if (!person) return source;
  const escaped = String(person).trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  if (!escaped) return source;
  const match = source.match(new RegExp(escaped, 'iu'));
  if (!match || match.index == null) return source;
  const start = Math.max(0, match.index - 40);
  const end = Math.min(source.length, match.index + escaped.length + 40);
...
export function correctDebtDirections(text, transactions) {
  ...
  // رسالة فيها معاملة واحدة بس: نفس المنطق القديم كامل على النص كله.
  if (transactions.length === 1) {
    const only = transactions[0];
    if (!only || only.type !== 'debt') return transactions;
    const owedToUser = DEBT_OWED_TO_USER_PATTERNS.some((p) => p.test(source));
    const owedByUser = DEBT_OWED_BY_USER_PATTERNS.some((p) => p.test(source));
    if (owedToUser && !owedByUser) return [{ ...only, direction: 'lent' }];
    if (owedByUser && !owedToUser) return [{ ...only, direction: 'borrowed' }];
    return transactions;
  }

  // رسالة فيها كذا معاملة: نطبّق نفس التحقق لكل معاملة دين لوحدها، على "شريحة" من النص حوالين
  // اسم الشخص بتاعها بس (مش الرسالة كلها) — عشان لو فيه أكتر من دين لأشخاص مختلفين في نفس
  // الرسالة، كل واحد ياخد اتجاهه الصح من كلامه هو مش من كلام دين تاني جنبه بالغلط.
  return transactions.map((tx) => {
    if (!tx || tx.type !== 'debt') return tx;
    const window = localWindowAroundPerson(source, tx.person);
    const owedToUser = DEBT_OWED_TO_USER_PATTERNS.some((p) => p.test(window));
    const owedByUser = DEBT_OWED_BY_USER_PATTERNS.some((p) => p.test(window));
    if (owedToUser && !owedByUser) return { ...tx, direction: 'lent' };
    if (owedByUser && !owedToUser) return { ...tx, direction: 'borrowed' };
    return tx;
  });
}
...
const DEBT_VERB_NEAR_PERSON = /(?:عطيت|اديت|أديت|سلفت|استلفت|اخدت|أخدت|واصل|دفعت\s+لـ?|حولت\s+لـ?)/iu;

// ============ تصحيح "دين اتصنف غلط كمصروف" في الرسايل المتعددة البنود ============
// المشكلة اللي كانت بتحصل: رسالة فيها "أعطيت لمحمد أحمد 500" جوه رسالة أطول فيها مصاريف تانية،
// والموديل كان أحيانًا يرجعها expense بفئة عشوائية (زي "مواصلات") بدل debt، لأنه مركّز على باقي
// البنود. هنا بنفحص كل معاملة expense لوحدها: لو الجزء اللي حواليها في النص فيه فعل دين واضح
// (عطيت/سلفت/واصل...) قبلها مباشرة، نرجّعها لنوعها الصح (debt) بدل ما نسيبها متسجلة غلط كمصروف.
export function correctExpenseMisclassifiedAsDebt(text, transactions) {
  const source = normalizeDigits(text);
  if (!Array.isArray(transactions) || transactions.length < 2) return transactions;

  return transactions.map((tx) => {
    if (!tx || tx.type !== 'expense' || !Number.isFinite(Number(tx.amount))) return tx;
    // نبني نافذة نص حوالين نفس الرقم بتاع المعاملة دي (مش الرسالة كلها) عشان الفحص يكون محلي.
    const amountStr = String(tx.amount);
    const idx = source.indexOf(amountStr);
    if (idx === -1) return tx;
    const window = source.slice(Math.max(0, idx - 40), idx);
    if (!DEBT_VERB_NEAR_PERSON.test(window)) return tx;

    // فيه فعل دين واضح قبل الرقم مباشرة، فده على الأرجح دين اتصنف غلط كمصروف — نستخرج اسم
    // الشخص من الكلمات بين الفعل والرقم كـ "أفضل تخمين" بدل ما نسيب الدين ضايع كمصروف بفئة غلط.
    const personGuess = window
      .replace(DEBT_VERB_NEAR_PERSON, ' ')
      .replace(/^(?:ل|لـ|إلى|الى)\s*/u, ' ')
      .trim();
    return {
      type: 'debt',
      person: personGuess || tx.note || 'غير محدد',
      amount: tx.amount,
      currency_code: tx.currency_code || 'EGP',
      direction: 'lent', // أفعال زي عطيت/سلفت/واصل لـ = المستخدم صاحب الحق
      is_repayment: false,
      note: tx.note || '',
...
// ============ finalizeTransactions: خطوة أخيرة إلزامية بعد كل التصحيحات (اتجاه دين / دين اتصنف
// غلط كمصروف / إلخ) — لازم تتنادى في الآخر عشان displayLabel يتبني من القيم النهائية المصححة،
// مش من القيم القديمة قبل التصحيح (وإلا هيفضل يوريّ فئة أو شخص غلط على الكارت). ============
export function finalizeTransactions(transactions) {
  if (!Array.isArray(transactions)) return transactions;
  return transactions.map((tx) => (tx && typeof tx === 'object' ? { ...tx, displayLabel: generateDisplayLabel(tx) } : tx));
}

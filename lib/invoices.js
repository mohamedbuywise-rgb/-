...
export async function recordInvoice(receipt, userId, chatId, extraFooter = '') {
  ...
  await sendInvoiceConfirmation(receipt, saved.invoiceId, userId, chatId, extraFooter);
  return saved.invoiceId;
}

// ============ رسالة تأكيد واحدة بكل الأصناف — بديل عن رسالة "تمام سجلت المصروف" العادية ============
async function sendInvoiceConfirmation(receipt, invoiceId, userId, chatId, extraFooter) {
  const context = await getUserGlobalContext(userId);
  ...
  for (const it of receipt.items) {
    ...
    msg += `${emoji} ${it.name} — ${formatMoney(it.amount, context)}\n`;
  }

  msg += `\n💰 <b>الإجمالي:</b> ${formatMoney(receipt.totalAmount, context)}`;

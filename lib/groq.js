...
1. "expense" (مصروف استهلاكي عادي):
   {"type": "expense", "amount": رقم, "category": "واحدة من: ${CATEGORIES.join(', ')}", "note": "وصف حقيقي", "raw_text": "العبارة الأصلية"}
2. "income" (دخل/بيع/تحصيل أموال):
   {"type": "income", "amount": رقم, "category": "دخل/بيع خدمة/راتب/هدايا أو أقرب وصف", "note": "ماذا باع أو حصل", "raw_text": "العبارة الأصلية"}
3. "purchase" (شراء شيء محدد، ويظل خارج تصنيف المصروفات العامة إذا كان جهازًا أو أصلًا):
   {"type": "purchase", "amount": رقم, "category": "الفئة المناسبة", "item": "اسم الشيء أو الموديل", "note": "وصف الشراء", "raw_text": "العبارة الأصلية"}
4. "asset" (شراء أصل/جهاز/معدات مرتفعة القيمة):
   {"type": "asset", "amount": رقم, "category": "أجهزة/معدات أو أقرب فئة", "item": "اسم الأصل أو الموديل", "note": "وصف الأصل", "raw_text": "العبارة الأصلية"}
5. "transfer" (تحويل بين حسابات المستخدم أو تحويل مالي لا يُعد مصروفًا):
   {"type": "transfer", "amount": رقم, "note": "مصدر التحويل ووجهته", "raw_text": "العبارة الأصلية"}
6. "refund" (استرداد مبلغ):
   {"type": "refund", "amount": رقم, "category": "الفئة المناسبة", "note": "ما تم استرداده", "raw_text": "العبارة الأصلية"}
...
- معنى الاتجاه بالمصري لازم يتحدد من صاحب الحق، وليس من كلمة "واصل" وحدها:
  * direction = "lent" = ليك عند الشخص / الشخص مديون لك / الفلوس خرجت منك: "أديت محمد"، "ديت محمد"، "سلفت محمد"، "محمد خد مني"، "محمد استلف مني"، "ليّا عند محمد"، "واصل لي عند محمد"، "محمد عليه لي"، "واصل له منّي".
  * direction = "borrowed" = عليك للشخص / أنت مديون له / الفلوس دخلت لك منه: "استلفت من محمد"، "خدت من محمد"، "محمد أداني"، "محمد سلفني"، "عليّا لمحمد"، "واصل له عندي".
  * عبارة "واصل لي" أو "واصل لي عند فلان" تعني أن المبلغ مستحق للمستخدم عند فلان، إذن direction = "lent".
  * عبارة "واصل له منّي" تعني أن المبلغ مستحق للشخص من المستخدم، إذن direction = "lent"؛ أما "واصل له عندي" فتعني أن المستخدم عليه للشخص، إذن direction = "borrowed".
  * "واصل من فلان" في الاستعمال المصري تعني غالبًا أن المبلغ مستحق للمستخدم عند فلان، إذن direction = "lent"؛ ولا تجعلها borrowed إلا إذا ظهرت قرينة صريحة مثل "استلفت من" أو "خدت من". أما "واصل له عندي" فتعني borrowed.
...
- "واصل من فلان" تعني direction: "lent" لأن الحق للمستخدم عند فلان، و"واصل لفلان" تعني direction: "borrowed" إذا كان المقصود أن المستخدم عليه المبلغ له. لا تعتمد على كلمة "واصل" وحدها بل على صاحب الحق.
...
"واصل له منّي 300 جنيه" -> {"transactions": [{"type":"debt","person":"غير محدد","amount":300,"direction":"lent","is_repayment":false,"note":""}]}

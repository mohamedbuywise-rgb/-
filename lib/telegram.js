...
// ============ إرسال صورة من بيانات خام (Buffer/base64) — مستخدمة لبعت إثبات دفع مرفوع من صفحة ويب (مش من تليجرام نفسه) ============
export async function sendTelegramPhotoBuffer(chatId, buffer, filename, caption, parseMode) {
  ...
  if (!data || !data.ok) {
    await handleFailedTelegramCall(chatId, data, res);
    throw new Error(`sendPhotoBuffer failed: ${JSON.stringify(data)}`);
  }
  return data;
}
...
// ============ تسجيل الـ webhook عند تليجرام مع secret_token ============
// بيقول لتليجرام: "ابعت كل التحديثات على webhookUrl ده، وحط secretToken في هيدر
// X-Telegram-Bot-Api-Secret-Token مع كل طلب" — عشان api/telegram-webhook.js يتأكد إن
// الطلب فعلاً جاي من تليجرام مش من حد عرف الـ URL وبيبعتله طلبات مزوّرة.
export async function setWebhookSecret(webhookUrl, secretToken) {

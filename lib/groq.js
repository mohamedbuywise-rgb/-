...
import { assistantContextBlock } from './globalContext.js';
...
export async function transcribeVoice(fileId) {
  ...
  // لا نفرض ar هنا: Whisper يكتشف العربية/الإنجليزية واللهجات تلقائيًا.
...
export async function classifyMessage(text, context = {}) {
  const prompt = `حدد نوع المعاملات في الرسالة الجاية من مستخدم عربي أو إنجليزي من أي دولة. استخدم السياق التالي عند فهم العملة والعبارات المحلية: ${assistantContextBlock(context)}. الرسالة ممكن يكون فيها معاملة واحدة أو أكتر.
...
// ============ محلل التسجيل الموحد للداشبورد (نص أو نص مفرغ من الصوت) ============
export async function parseUnifiedEntryText(text, context) {
  const prompt = `You are Dabbar's financial entry parser for users worldwide. Return ONLY valid JSON, no markdown.
${assistantContextBlock(context)}
Adapt to the user's dialect and wording. Recognize regional currency names and colloquial terms (for example AED/dirham, SAR/riyal, KWD/dinar, EGP/pound) without converting currencies or guessing missing amounts.
User input: ${JSON.stringify(String(text || '').slice(0, 1200))}

Classify the input as exactly one type: expense, debt, or invoice.
Return this shape:
{"type":"expense|debt|invoice","confidence":0.0,"amount":number|null,"category":string|null,"note":string|null,"person":string|null,"direction":"lent|borrowed|null","merchant":string|null,"items":[{"name":string,"amount":number,"category":string}],"needsConfirmation":true}
  Rules: amount must be numeric only; never guess a missing amount; use null when missing. Preserve the user's original note. For expense category choose one of: ${CATEGORIES.join(', ')}. For invoice, items may be empty only if the input has no item detail. For debt, person and direction are required when present. The result is a draft only and must always require confirmation.`;
  const reply = await askGroqText(prompt, { temperature: 0, maxTokens: 500 });
  try {
    const parsed = JSON.parse(reply);
    const type = ['expense', 'debt', 'invoice'].includes(parsed.type) ? parsed.type : null;
    if (!type || !parsed.amount || Number(parsed.amount) <= 0) return { success: false, error: 'محتاج مبلغ واضح عشان أفهم العملية.' };
    return {
      success: true,
      draft: {
        type,
        ...
        sourceText: String(text || '').slice(0, 1200),
      },
    };
  } catch (error) {
    console.error('parseUnifiedEntryText invalid JSON:', error);
    return { success: false, error: 'معرفتش أفهم التسجيل. جرّب تقول المبلغ والفئة بوضوح.' };
  }
}

export async function transcribeAudioBase64(audioBase64, mimeType = 'audio/webm', language = null) {
  const clean = String(audioBase64 || '').replace(/^data:[^;]+;base64,/, '');
  if (!clean) return { success: false, error: 'ملف الصوت فاضي.' };
  try {
    const bytes = Buffer.from(clean, 'base64');
    const formData = new FormData();
    formData.append('file', new Blob([bytes], { type: mimeType }), 'dabbar-voice.webm');
    formData.append('model', 'whisper-large-v3');
    // اللغة اختيارية: عدم فرض ar يحسن التعامل مع اللهجات الخليجية والإنجليزية.

...
export const PREFERENCE_COLUMNS = 'daily_reminder_enabled, daily_summary_enabled, weekly_summary_enabled, budget_alert_enabled, bank_movement_enabled, payment_reminders_enabled, daily_presence_enabled, daily_reminder_hour, daily_reminder_minute, budget_alert_threshold, timezone';
export const LEGACY_PREFERENCE_COLUMNS = 'daily_reminder_enabled, daily_summary_enabled, weekly_summary_enabled, budget_alert_enabled, bank_movement_enabled, payment_reminders_enabled, daily_reminder_hour, daily_reminder_minute, budget_alert_threshold, timezone';

function isMissingDailyPresenceColumn(error) {
  const text = JSON.stringify(error || {}).toLowerCase();
  return text.includes('daily_presence_enabled') && (text.includes('schema cache') || text.includes('does not exist') || text.includes('pgrst204') || text.includes('pgrst205'));
}
...
export async function getNotificationPreferences(telegramUserId) {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error && isMissingDailyPresenceColumn(error)) {
    // توافق مؤقت مع قواعد البيانات التي لم تُشغّل migration الخاصة بالخيار السابع بعد.
...
export async function saveNotificationPreferences({ authUserId, telegramUserId, preferences }) {
  ...
  let { data, error } = await supabase
    ...
    .single();
  if (error && isMissingDailyPresenceColumn(error)) {
    // نحفظ بقية الإعدادات فورًا حتى قبل تشغيل migration؛ الخيار السابع يظل افتراضيًا مفعّلًا.
    const legacyPayload = { ...payload };
    delete legacyPayload.daily_presence_enabled;
...
//   await sendBankMovementPush(userId, { direction: 'out' | 'in', amount, currency, merchant, bank, balance, dedupeKey })
// dedupeKey (اختياري بس مهم): أي مفتاح ثابت للحركة (id الحركة أو هاش نص الرسالة) عشان لو MacroDroid/الشورتكات بعتت نفس
// الرسالة مرتين، الإشعار ما يتبعتش مرتين.
export async function sendBankMovementPush(telegramUserId, movement = {}) {
  try {
    ...
    const kind = String(movement.direction ?? movement.type ?? movement.kind ?? '').toLowerCase();
    const incoming = ['in', 'incoming', 'income', 'credit', 'deposit', 'received', 'إيداع', 'دخل', 'وارد'].includes(kind);
    const currency = String(movement.currency || 'جنيه').trim();

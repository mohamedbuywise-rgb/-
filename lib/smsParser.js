...
// ---------------------------------------------------------------- التاجر / الطرف الآخر
// ============ آخر ٤ أرقام الحساب/الكارت من نص الـ SMS (بيدعم أرقام عربية وإنجليزية) ============
// أمثلة: "من حسابك ***1234" / "بطاقة تنتهي بـ 1234" / "account ending 1234" / "حساب رقم 100200301234" / "A/C XXXX1234"
function extractAccountLast4(text) {
  const t = String(text || '').replace(/[٠-٩]/g, (d) => String('٠١٢٣٤٥٦٧٨٩'.indexOf(d)));
  const patterns = [
    /(?:[*xX•●]{2,}|\.{3})\s*(\d{4})\b/,
    /(?:ending|ends)\s*(?:in|with|with\s*digits)?\s*[:\-]?\s*(\d{4})\b/i,
    /(?:تنتهي|ينتهي|المنتهي|المنتهيه|المنتهية|تنتهى|منتهي)\s*(?:ب|بـ|بال)?\s*(\d{4})\b/,
    /(?:حساب(?:ك)?|الحساب|بطاق[هة]|بطاقتك|كارت|كارتك|card|account|a\/c|acct)\s*(?:رقم|no\.?|number|#)\s*[:\-]?\s*[*xX•●]*\s*(\d{4,20})\b/i,
  ];
  for (const rx of patterns) {
    const m = t.match(rx);
    if (m && m[1]) return m[1].slice(-4);
  }
  return '';
}
...
export function parseBankSms(rawText, { sender = '' } = {}) {
  ...
  // آخر أرقام الحساب/الكارت (عشان نفرّق بين حسابين في نفس البنك)
  const accountLast4 = extractAccountLast4(cleaned) || cardLast4;
  ...
  const done = (item, confidence = 'high', reason = '') => ({
    skip: null,
    confidence,
    items: [{ ...base, ...item, balance: meta.balance, fee: meta.fee, reference, card_last4: cardLast4, account_last4: accountLast4 }],
    meta: { ...meta, reason },

...
export function normalizeFinancialTransaction(transaction, sourceText = '') {
  ...
  return { ...tx, amount, currency_code, raw_text: tx.raw_text || String(sourceText || '').slice(0, 2500) };
}

// ============ ملحوظة مهمة بعد إصلاح باگ "تصادم المبالغ" ============
// الدالة دي بترقّع رسالة واحدة قصيرة لو الموديل رجّع نوع/فئة غير معروفة (مثلاً unknown) رغم إن فيها
// مبلغ واضح فعلاً. **لازم متلمسش مبلغ أو فئة رجعهم الموديل بالفعل بثقة** — لأن الاستخراج الحتمي هنا
// (extractDeterministicExpense) بيدور على أول رقم بس في النص، ولو النص فيه أكتر من رقم (زي رسالة فيها
// أكتر من مصروف/دين مع بعض) ممكن يمسك رقم غلط تمامًا ويكتب فوق رقم صح جاله من التصنيف الذكي.
// فالقاعدة الوحيدة الصارمة المتبقية هنا: منستخدمش الـ hint إلا لو المبلغ الأصلي من الموديل فعلاً
// مفقود/غير صالح، ومنغيّرش فئة موجودة بالفعل.
export function reconcileSingleTransaction(transactions, sourceText = '') {
  ...
  const existingAmountValid = Number.isFinite(Number(existing?.amount)) && Number(existing.amount) > 0;
  const knownType = ['expense', 'income', 'purchase', 'asset', 'transfer', 'refund', 'debt', 'settlement', 'portfolio_buy', 'portfolio_sell'].includes(existing?.type);

  // لو الموديل رجّع مبلغ ونوع صحيحين خالص، متلمسهمش خالص — سيبهم زي ما هما.
  if (existingAmountValid && knownType) return transactions;
  ...
  return [{
    ...existing,
    type: knownType ? existing.type : hint.type,
    // منغيّرش مبلغ صحيح موجود بالفعل، بنستخدم الـ hint بس لو المبلغ الأصلي مفقود/غير صالح.
    amount: existingAmountValid ? existing.amount : hint.amount,
    currency_code: existing.currency_code || hint.currency_code || 'EGP',
    category: (!existing.category && (existing.type === 'expense' || !knownType)) ? hint.category : existing.category,
    note: existing.note || hint.note,
    raw_text: existing.raw_text || String(sourceText || '').slice(0, 2500),
  }];
}
...
const NON_PERSON_DEBT_COUNTERPARTIES = [
  { category: 'مواصلات', words: ['delivery', 'deliveroo', 'talabat', 'طلبات', 'دليفري', 'ديليفري', 'taxi', 'تاكسي', 'uber', 'اوبر', 'أوبر', 'careem', 'كريم', 'مواصلات', 'transport', 'transportation', 'بنزين', 'مترو', 'ميكروباص', 'اتوبيس'] },
  { category: 'أكل', words: ['restaurant', 'مطعم', 'food', 'اكل', 'أكل', 'غدا', 'غداء', 'فطار', 'عشا'] },
  { category: 'فواتير', words: ['electricity', 'water', 'internet', 'فاتورة', 'فواتير', 'كهربا', 'كهرباء', 'مياه', 'غاز', 'انترنت', 'إنترنت'] },
];

function debtPersonIsNonPersonService(person) {
  const normalizedPerson = String(person || '').toLowerCase().trim();
  if (!normalizedPerson || normalizedPerson === 'غير محدد' || normalizedPerson === 'غير معروف') return null;
  for (const group of NON_PERSON_DEBT_COUNTERPARTIES) {
    if (group.words.some((word) => containsWholePhrase(normalizedPerson, word))) return group.category;
  }
  return null;
}

// النموذج قد يلتقط «أخذ مني Delivery 80» كدين لأن فيها فعلًا يشبه أخذ المال.
// لو الطرف المستخرج اسم خدمة/فئة وليس شخصًا، نحوله حتميًا لمصروف ونحتفظ بباقي التفاصيل.
export function correctNonPersonDebtCounterparties(text, transactions) {
  if (!Array.isArray(transactions)) return transactions;
  return transactions.map((transaction) => {
    if (transaction?.type !== 'debt') return transaction;
    const category = debtPersonIsNonPersonService(transaction.person);
    if (!category) return transaction;
    return {
      ...transaction,
      type: 'expense',
      category,
      note: transaction.note || String(transaction.person || category),
    };
  });
}
...
export function extractDeterministicExpense(text) {
  ...
  // ده استخراج احتياطي "آخر خط دفاع" لرسالة فيها معاملة واحدة بس وفشل الموديل يفهمها.
  // لو النص فيه أكتر من رقم، بقى غير موثوق فيه إننا نخمّن مين الرقم المقصود (ده بالظبط
  // اللي كان بيسبب مبلغ غلط في رسايل فيها كذا مصروف مع بعض) — فبنرفض تمامًا في الحالة دي
  // ونسيب الأمر للتصنيف الذكي (classifyMessage) اللي أصلًا مصمم يفهم كذا معاملة في رسالة واحدة.
  const allNumbers = source.match(/[0-9]+(?:[.][0-9]+)?/g) || [];
  if (allNumbers.length > 1) return null;

  // يدعم: «أكل 500»، «مواصلات 300»، «فطار 100 جنيه»، «500 أكل»، «300 مواصلات»،
  ...
  if (hasIncomeIntent) {
    ...
    return { type: 'income', amount, currency_code, category: onlineIncome ? 'دخل/أونلاين' : 'دخل', note, raw_text: String(text || '').slice(0, 2500) };
  }
  ...
  return { type: 'expense', amount, category, currency_code, note, raw_text: String(text || '').slice(0, 2500) };
}

...
export async function getDebtHistoryData(userId) {
  ...
  return { people: Object.entries(people).map(([key, p]) => ({ name: p.name, active: !latest[key] || p.transactions.some((t) => t.createdAt > latest[key]), lastSettlement: latest[key] || null })).sort((a,b) => a.name.localeCompare(b.name, 'ar')), settlements: (settlements || []).map((s) => ({ person_name: s.person_name, settled_at: s.settled_at })).reverse() };
}

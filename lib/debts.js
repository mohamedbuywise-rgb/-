...
export async function recordDebt(debt, userId, chatId, options = {}) {
...
export async function updateDebtById(debtId, userId, updates = {}) {
  ...
  if (updates.due_date !== undefined) patch.due_date = updates.due_date || null; // فاضي = مسح تاريخ الاستحقاق
  ...
  const { data, error } = await supabase
    ...
    .select('id, person_name, amount, currency_code, due_date')
    .single();
...
// ============ حذف كشف حساب شخص متصفّي بالكامل (كل الديون + سجلات التصفية) — بيتنادى من زرار الحذف في تبويب "كشف الحساب" بالداشبورد ============
// أمان: بنسمح بالحذف بس لو صافي الرصيد صفر فعلاً (يعني الشخص متصفّي بالفعل)، عشان محدش يمسح كشف حساب فيه ديون لسه نشطة بالغلط.
export async function deletePersonDebtHistory(userId, personName) {
  const { data: debts, error: debtsError } = await supabase
    .from('debts')
    .select('id, amount, direction')
    .eq('telegram_user_id', userId)
    .ilike('person_name', personName);
  if (debtsError) { console.error('deletePersonDebtHistory debts select error:', JSON.stringify(debtsError)); return { ok: false, error: 'حصل خطأ وإحنا بنتأكد من الحساب.' }; }
  if (!debts || debts.length === 0) return { ok: false, error: 'معندكش أي عمليات مسجلة مع الشخص ده.' };

  const net = debts.reduce((sum, d) => sum + (d.direction === 'borrowed' ? -Number(d.amount) : Number(d.amount)), 0);
  if (Math.round(net * 100) !== 0) return { ok: false, error: 'الحساب لسه نشط ومفيهوش تصفية كاملة، فمينفعش نمسحه.' };

  const { error: deleteDebtsError } = await supabase.from('debts').delete().eq('telegram_user_id', userId).ilike('person_name', personName);
  if (deleteDebtsError) { console.error('deletePersonDebtHistory delete debts error:', JSON.stringify(deleteDebtsError)); return { ok: false, error: 'معرفناش نحذف كشف الحساب دلوقتي.' }; }

  const { error: deleteSettlementsError } = await supabase.from('debt_settlements').delete().eq('telegram_user_id', userId).ilike('person_name', personName);
  if (deleteSettlementsError) console.error('deletePersonDebtHistory delete settlements error:', JSON.stringify(deleteSettlementsError));
...
export async function computeNetByPerson(userId) {
  const { data: debts, error: debtsError } = await supabase
    .from('debts')
    .select('person_name, amount, currency_code, direction, created_at, due_date')
    .eq('telegram_user_id', userId)
    .eq('commitment_type', 'debt'); // الجمعية والمناسبات ليهم حساب منفصل، مش بيدخلوا في صافي الديون العادي
  ...
  const netByPerson = {}; // key: الاسم الأصلي (أول ظهور), value: { net, displayName, nearestDueDate }
  for (const d of debts || []) {
    ...
    if (!netByPerson[key]) netByPerson[key] = { net: 0, displayName: d.person_name, byCurrency: {}, nearestDueDate: null };
    ...
    // أقرب تاريخ استحقاق مش فات لسه (أو لو كلهم فاتوا، أقدم واحد فات — عشان يبان "متأخر")
    if (d.due_date && (!netByPerson[key].nearestDueDate || d.due_date < netByPerson[key].nearestDueDate)) {
      netByPerson[key].nearestDueDate = d.due_date;
    }
  }
...
// ============ تسجيل إن تذكير الديون القديمة اتبعت دلوقتي لمجموعة أشخاص (بيتستخدم بعد إرسال sendOldDebtsReminder) ============
// بيحدّث last_reminded_at لكل شخص، عشان الكولداون في getOldUnsettledDebtsSummary يمنع تكرار نفس التذكير قبل 7 أيام.
export async function recordDebtReminders(userId, personNames = []) {
  ...
  if (error) console.error('recordDebtReminders error:', JSON.stringify(error));
}

// ============ آخر دين مسجل للمستخدم (قراءة بس، من غير حذف) — بيتستخدم في تأكيد "امسح آخر دين" ============
export async function getMostRecentDebt(userId) {  const { data } = await supabase
    .from('debts')
...
// ============================================================================
// ============ الجمعية (Gam3eya) — نفس جدول debts، commitment_type = 'gameya' ============
// ============================================================================

// ============ إنشاء جمعية جديدة ============
export async function createGameya({ groupName, installment, groupSize, myTurn, startDate, memberNames }, userId) {
  const cleanGroupSize = Number(groupSize);
  const cleanMyTurn = Number(myTurn);
  const cleanInstallment = Number(installment);
  if (!groupName || !cleanInstallment || cleanInstallment <= 0) return { error: 'محتاج اسم الجمعية وقسط شهري صحيح.' };
  if (!cleanGroupSize || cleanGroupSize < 2) return { error: 'عدد الأفراد لازم يكون 2 على الأقل.' };
  if (!cleanMyTurn || cleanMyTurn < 1 || cleanMyTurn > cleanGroupSize) return { error: 'دورك لازم يكون رقم من ضمن عدد الأفراد.' };
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      amount: cleanInstallment,
      direction: 'borrowed', // بندفع القسط شهريًا (زي إننا "مديونين" للجمعية) لحد ما يجيلنا دورنا
      commitment_type: 'gameya',
      commitment_meta: {
    ...
    .select('id, person_name, amount, commitment_meta')
    .single();

  if (error) {
    console.error('createGameya error:', JSON.stringify(error));
    return { error: 'حصل خطأ وإحنا بنضيف الجمعية، جرب تاني.' };
  }
  return { gameya: data };
}

// ============ حساب تاريخ دور الاستلام المتوقع وباقي الشهور ============
function computeGameyaTurnInfo(meta, installment) {
  const startDate = meta.startDate ? new Date(meta.startDate) : new Date();
  const monthsUntilTurn = Math.max(Number(meta.myTurn) - 1, 0);
  const turnDate = new Date(startDate);
  turnDate.setMonth(turnDate.getMonth() + monthsUntilTurn);
  const today = new Date();
  const monthsElapsed = Math.max(
    (today.getFullYear() - startDate.getFullYear()) * 12 + (today.getMonth() - startDate.getMonth()),
    0
  );
  const monthsRemaining = Math.max(monthsUntilTurn - monthsElapsed, 0);
  const totalReceiveAmount = Number(installment) * Number(meta.groupSize || 1);
  return {
    turnDate: turnDate.toISOString().slice(0, 10),
    monthsRemaining,
    monthsElapsed: Math.min(monthsElapsed, monthsUntilTurn),
    totalReceiveAmount,
    hasReceivedTurn: monthsElapsed >= monthsUntilTurn,
  };
}

// ============ كل الجمعيات النشطة للمستخدم مع تفاصيل الدور المحسوبة ============
export async function getGameyaList(userId) {
  const { data, error } = await supabase
    .from('debts')
    .select('id, person_name, amount, commitment_meta, created_at')
    .eq('telegram_user_id', userId)
    .eq('commitment_type', 'gameya')
    .order('created_at', { ascending: false });
  if (error) { console.error('getGameyaList error:', JSON.stringify(error)); return []; }

  return (data || []).map((row) => {
    const meta = row.commitment_meta || {};
    const turnInfo = computeGameyaTurnInfo(meta, row.amount);
    return {
      id: row.id,
      groupName: row.person_name,
      installment: Number(row.amount),
      groupSize: meta.groupSize,
      myTurn: meta.myTurn,
      startDate: meta.startDate,
      memberNames: meta.memberNames || [],
      paidThisMonth: meta.paidThisMonth || [],
      ...turnInfo,
    };
  });
}

// ============ تبديل حالة دفع عضو معيّن للقسط الشهري (toggle) ============
export async function toggleGameyaMemberPaid(gameyaId, memberName, userId) {
  const { data: row, error: fetchError } = await supabase
    .from('debts')
    .select('commitment_meta')
    .eq('id', gameyaId).eq('telegram_user_id', userId).eq('commitment_type', 'gameya')
    .maybeSingle();
  if (fetchError || !row) return { error: 'الجمعية دي مش موجودة.' };

  const meta = row.commitment_meta || {};
  const paidThisMonth = new Set(meta.paidThisMonth || []);
  if (paidThisMonth.has(memberName)) paidThisMonth.delete(memberName);
  else paidThisMonth.add(memberName);

  const { error } = await supabase
    .from('debts')
    .update({ commitment_meta: { ...meta, paidThisMonth: Array.from(paidThisMonth) } })
    .eq('id', gameyaId).eq('telegram_user_id', userId);
  if (error) return { error: 'حصل خطأ، جرب تاني.' };
  return { ok: true, paidThisMonth: Array.from(paidThisMonth) };
}

export async function deleteGameya(gameyaId, userId) {
  const { error } = await supabase.from('debts').delete().eq('id', gameyaId).eq('telegram_user_id', userId).eq('commitment_type', 'gameya');
  if (error) console.error('deleteGameya error:', error);
}

// ============================================================================
// ============ المناسبات الاجتماعية (أفراح/عزومات/سبوع) — commitment_type = 'occasion' ============
// ============================================================================

// ============ تسجيل "نقطة"/هدية مناسبة — منسجلهاش كدين حقيقي، بس بنسمح نربطها كمديونية اختيارية ============
export async function createOccasion({ personName, amount, occasionType, note, asDebt, direction }, userId) {
  const cleanAmount = Number(amount);
  if (!personName || !cleanAmount || cleanAmount <= 0) return { error: 'محتاج اسم الشخص ومبلغ صحيح.' };
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      person_name: personName,
      amount: cleanAmount,
      // لو asDebt=true (يعني إحنا معتبرينها "دين" هيرجع فعلي)، بتتحط في commitment_type='debt'
      // عادي عشان تدخل في حساب الصافي، وبتبقى مربوطة بمصدرها (المناسبة) في commitment_meta فقط للعرض.
      // لو مش asDebt، بتفضل 'occasion' بحتة (توثيق وتذكرة بس، مش بتدخل في صافي الديون).
      commitment_type: asDebt ? 'debt' : 'occasion',
      direction: asDebt ? (direction === 'borrowed' ? 'borrowed' : 'lent') : 'lent',
      note: note || '',
      commitment_meta: { occasionType: occasionType || 'مناسبة', linkedToDebt: Boolean(asDebt) },
    })
    .select('id, person_name, amount, commitment_meta')
    .single();

  if (error) {
    console.error('createOccasion error:', JSON.stringify(error));
    return { error: 'حصل خطأ وإحنا بنسجل المناسبة، جرب تاني.' };
  }
  return { occasion: data };
}

// ============ ملخص كل المناسبات: إجمالي وتجميع حسب الشخص ============
export async function getOccasionsSummary(userId) {
  const { data, error } = await supabase
    .from('debts')
    .select('id, person_name, amount, commitment_meta, direction, created_at')
    .eq('telegram_user_id', userId)
    .or('commitment_type.eq.occasion,commitment_meta->>linkedToDebt.eq.true')
    .order('created_at', { ascending: false });
  if (error) { console.error('getOccasionsSummary error:', JSON.stringify(error)); return { total: 0, items: [], byPerson: {} }; }

  const items = (data || []).map((row) => ({
    id: row.id,
    personName: row.person_name,
    amount: Number(row.amount),
    occasionType: row.commitment_meta?.occasionType || 'مناسبة',
    linkedToDebt: Boolean(row.commitment_meta?.linkedToDebt),
  ...
  for (const it of items) {
    byPerson[it.personName] = (byPerson[it.personName] || 0) + it.amount;
  }
  return { total, items, byPerson };
}

export async function deleteOccasion(occasionId, userId) {
  const { error } = await supabase.from('debts').delete().eq('id', occasionId).eq('telegram_user_id', userId);
  if (error) console.error('deleteOccasion error:', error);
}

...
export async function recordDebt(debt, userId, chatId) {
  ...
  if (isRepayment) {
    summary = isLent
      ? `↩️ رجّعت لـ <b>${debt.person}</b> ${formatMoney(debt.amount, context)}`
      : `↩️ <b>${debt.person}</b> رجّعلك ${formatMoney(debt.amount, context)}`;
  } else {
    summary = isLent
      ? `📤 بقى ليك عند <b>${debt.person}</b> ${formatMoney(debt.amount, context)}`
      : `📥 بقى عليك لـ <b>${debt.person}</b> ${formatMoney(debt.amount, context)}`;
  }

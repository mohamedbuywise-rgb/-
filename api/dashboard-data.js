...
export default async function handler(req, res) {
  ...
  try {
    ...
    const telegramUserId = link?.telegram_user_id || await resolveDataUserId(userData.user);
    if (!telegramUserId) return res.status(500).json({ error: 'تعذر تجهيز مساحة بيانات الحساب حاليًا.' });
    console.log('dashboard-data: data user id:', telegramUserId, 'telegram linked:', Boolean(link));
    const globalContext = await getUserGlobalContext(telegramUserId);
    ...
    if (req.query.invoiceId) {
      ...
      return res.status(200).json({ linked: Boolean(link), invoice });
    }
    if (req.query.invoices) {
      const invoices = await getInvoicesList(telegramUserId);
      return res.status(200).json({ linked: Boolean(link), invoices });
    }

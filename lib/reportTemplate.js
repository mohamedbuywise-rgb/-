...
function buildDonutSvg(sortedCategories, totalLabel) {
  ...
  <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="${cx}" y="${cy - 5}" text-anchor="middle" font-family="FloosyFont, sans-serif" font-weight="700" font-size="17" fill="#241C12">${esc(totalLabel)}</text>
    <text x="${cx}" y="${cy + 14}" text-anchor="middle" font-family="FloosyFont, sans-serif" font-size="9" fill="#726B57">جنيه</text>
  </svg>`;
}
...
// استايل "إيصال ورقي فاتح" (نفس هوية فلوسي بوت في الموقع ودليل الاستخدام)، مع تفاصيل كل عملية
...
<body>
  <div class="sheet">
    <div class="brand">تقرير فلوسي 💰</div>

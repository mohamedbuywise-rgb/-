...
// خيار "silent": بيتستخدم لما بنسجل أكتر من مصروف مع بعض في نفس الرسالة/الفويس (زي "غدا 300 عشا 200 مواصلات 100").
// بدل ما كل مصروف يبعت رسالته الكاملة لوحده (وده كان بيسبب "spam" من رسائل تليجرام منفصلة لكل بند
// في الفويس الواحد)، لما silent=true بنعمل الإدراج في القاعدة بالظبط زي العادي (الدقة والفصل بين
// البنود في القاعدة متأثرتش خالص) لكن من غير ما نبعت رسالة أو زرار حذف — بنرجّع بيانات العملية
// المسجلة عشان الاستدعاء (handleIncomingText) يجمع كل البنود في رسالة واحدة مجمّعة بالفئة في الآخر.
export async function recordExpense(expense, text, userId, chatId, extraFooter = '', options = {}) {
  ...
  // لو silent، منبعتش أي رسالة أو نتاريخ Push هنا — بنرجّع البيانات بس، والاستدعاء اللي فوق
  // (handleIncomingText) هو اللي هيبني الرسالة المجمّعة ويبعتها مرة واحدة لكل البنود.
  if (silent) {
    return {
      id: inserted.id,
      category,
      amount: expense.amount,
      currency_code,
      moneyLabel: currencyLabel(currency_code),
      detail,
      isDuplicate,
      todayTotal,
    };
  ...
  await sendTelegramMessage(chatId, msg, 'HTML', {
    inline_keyboard: [[{ text: '🗑 حذف العملية دي', callback_data: `delexp:${inserted.id}` }]],
  });

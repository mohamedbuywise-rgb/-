...
// silent=true بيتستخدم لما الدين ده جزء من كذا معاملة مع بعض في نفس الرسالة/الفويس، عشان نجمعهم
// كلهم في رسالة واحدة بعدين بدل ما كل دين يبعت رسالته المستقلة (نفس مبدأ recordExpense بالظبط).
export async function recordDebt(debt, userId, chatId, options = {}) {
  ...
  await sendTelegramMessage(
    ...
    'HTML',
    { inline_keyboard: [[{ text: '🗑 حذف العملية دي', callback_data: `deldebt:${inserted.id}` }]] }
  );
}

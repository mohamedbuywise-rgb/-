...
// ============ إضافة "قرش النهاردة" — مرة واحدة بس في اليوم (بيتفحص بـ last_contribution_date) ============
export async function contributeDailyPiggybank(goalId, userId) {
  ...
  const newSaved = Number(row.saved_amount || 0) + Number(row.daily_amount || 0);
  const { data, error } = await supabase
    .from('goals')
    .update({ saved_amount: newSaved, last_contribution_date: today })

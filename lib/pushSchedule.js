...
async function sendDailyReminder(userId, prefs, slot) {
  ...
  const result = await sendClaimedPush(userId, claimType, slot.dateKey, { ...payload, url: APP_URL, icon: ICON, badge: BADGE }, { urgency: 'high' });
...
async function sendPaymentReminders(userId, prefs, slot, ctx) {
  ...
  for (const r of list) {
    ...
    const result = await sendClaimedPush(userId, 'payment-reminder', `${r.id}:${r.stage}`, {

...
import { getUserGlobalContext, formatMoney } from './globalContext.js';
...
async function checkPossibleDuplicate(userId, amount, category) {
...
export async function recordExpense(expense, text, userId, chatId, extraFooter = '') {
  ...
  const isDuplicate = await checkPossibleDuplicate(userId, expense.amount, category);
  ...
  const { data: todayExpenses } = await supabase
    ...
    .eq('telegram_user_id', userId)
    .eq('currency_code', context.currencyCode)
  ...
  let msg = `✅ <b>تمام، سجلت المصروف</b>\n${CATEGORY_EMOJI[category] || '📌'} ${category}${detail} · ${formatMoney(expense.amount, context)}\n\n💰 إجمالي صرفك النهاردة: <b>${formatMoney(todayTotal, context)}</b>`;
  if (isDuplicate) {

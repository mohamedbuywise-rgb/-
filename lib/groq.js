...
import { correctNonPersonDebtCounterparties } from './textNormalize.js';
...
- الدين لا يُسجّل إلا عندما يكون الطرف اسم شخص واضحًا: «أخذ مني أحمد 200» أو «أديت لمحمد 300» = debt. أسماء الخدمات والتجار والفئات ليست أشخاصًا أبدًا: «أخذ مني Delivery 80»، «Taxi أخذ مني 80»، «دفعت لمواصلات 80»، «أوبر 80» = expense عادي، والفئة المناسبة (مواصلات/أكل/فواتير). لا تضع Delivery أو Taxi أو مواصلات في حقل person.
...
الجملة: "${text}"`;
  ...
  try {
    ...
    if (parsed.transactions) return correctNonPersonDebtCounterparties(text, parsed.transactions);
    if (parsed.type) return correctNonPersonDebtCounterparties(text, [parsed]);
    return [{ type: 'unknown' }];
  } catch {
    return [{ type: 'unknown' }];
  }

...
async function handleEntryDraft(userId, body, res) {
  ...
  // لو الصوت جه من المسار المدمج (تفريغ+تصنيف في نداء واحد عبر Gemini) بيتملي هنا بالمعاملات جاهزة،
  // فمنعيدش نداء classifyMessage تاني على نفس النص (بيوفر round trip كامل غالبية الأوقات).
  let voicePreClassified = null;
  ...
  if (body.audioBase64) {
    ...
    voicePreClassified = transcript.transactions;
  }
  ...
  // لو الصوت رجع معاملات جاهزة من المسار المدمج (Gemini) بنستخدمها مباشرة من غير نداء تصنيف تاني.
  // لو فاضية (فشل المسار المدمج ورجعنا للمسار القديم بالفعل، أو مفيش معاملة واضحة) بنكمّل زي الإدخال النصي بالظبط.
  const parsed = (voicePreClassified && voicePreClassified.length) ? voicePreClassified : await classifyMessage(text);

...
const EXPENSE_CATEGORY_HINTS = [
  { category: 'أكل', words: ['اكل', 'أكل', 'غذا', 'غدا', 'غذاء', 'غداء', 'فطار', 'عشا', 'عشاء', 'مطعم', 'كشري', 'قهوة', 'كافيه', 'حلويات', 'سوبر ماركت', 'بقاله'] },
  { category: 'مواصلات', words: ['مواصلات', 'تاكسي', 'اوبر', 'أوبر', 'بنزين', 'مترو', 'ميكروباص', 'اتوبيس', 'أتوبيس'] },
  { category: 'تسوق', words: ['اشتريت', 'هدوم', 'ملابس', 'جزمه', 'جزمة', 'مشتريات', 'سوبر ماركت'] },
  { category: 'فواتير', words: ['فاتوره', 'فاتورة', 'كهربا', 'كهرباء', 'مياه', 'غاز', 'نت', 'انترنت', 'إنترنت'] },
  { category: 'صحة', words: ['دكتور', 'دواء', 'علاج', 'صيدليه', 'صيدلية', 'كشف'] },
  { category: 'شخصي وعناية', words: ['حلاق', 'حلاقة', 'قص شعر', 'كوافير'] },
  { category: 'ترفيه', words: ['سينما', 'فيلم', 'خروجه', 'خروجة', 'لعب'] },
];

const EXPENSE_INTENT_WORDS = [
  'صرفت', 'دفعت', 'اشتريت', 'حجزت', 'غذا', 'غدا', 'غذاء', 'غداء', 'اكل', 'أكل', 'فطار', 'عشا', 'عشاء',
  'مواصلات', 'تاكسي', 'اوبر', 'أوبر', 'بنزين', 'مطعم', 'قهوة', 'فاتورة', 'كهربا', 'دواء',
];
...
export function extractDeterministicExpense(text) {
  ...
  const amountMatch = source.match(/(?:^|[\s،,])([0-9]+(?:[.][0-9]+)?)[\s]*(?=جنيه|جنية|ج\.?م|جم|$|[،,])/u);
  if (!amountMatch) return null;

  const hasCurrency = /(?:جنيه|جنية|ج\.?م|جم)/u.test(source);
  const hasExpenseIntent = EXPENSE_INTENT_WORDS.some((word) => source.includes(word));
  const hasDebtIntent = DEBT_INTENT_WORDS.some((word) => source.includes(word));
  if (hasDebtIntent || (!hasCurrency && !hasExpenseIntent)) return null;

  const amount = Number(amountMatch[1]);
  if (!Number.isFinite(amount) || amount <= 0 || amount > 1000000000) return null;

  const categoryHint = EXPENSE_CATEGORY_HINTS.find(({ words }) => words.some((word) => source.includes(word)));
  const category = categoryHint?.category || 'أكل';
  const note = source
    .replace(amountMatch[0], ' ')
    .replace(/جنيه|جنية|ج\.?م|جم/gu, ' ')
    .replace(/\s+/g, ' ')
    .trim();

...
export async function addPortfolioAsset(userId, { name, subLabel, amount, alertThresholdPct, costBasis, quantity, unit }) {
  const cleanName = (name || '').trim();
  const cleanAmount = Number(amount);
  if (!cleanName || !cleanAmount || cleanAmount <= 0) {
    return { error: 'محتاج اسم الأصل وقيمة صحيحة أكبر من صفر.' };
  }
  const cleanThreshold = Number(alertThresholdPct);
  const hasThreshold = Number.isFinite(cleanThreshold) && cleanThreshold > 0;
  const cleanCostBasis = Number(costBasis);
  const hasCostBasis = Number.isFinite(cleanCostBasis) && cleanCostBasis > 0;
  const cleanQuantity = Number(quantity);
  const hasQuantity = Number.isFinite(cleanQuantity) && cleanQuantity > 0;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      name: cleanName,
      sub_label: (subLabel || '').trim() || null,
      amount: cleanAmount,
      alert_threshold_pct: hasThreshold ? cleanThreshold : null,
      cost_basis: hasCostBasis ? cleanCostBasis : null,
      quantity: hasQuantity ? cleanQuantity : null,
...
export async function updatePortfolioAsset(userId, assetId, { name, subLabel, amount, alertThresholdPct, quantity, unit }) {
  ...
  if (quantity !== undefined) {
    const cleanQuantity = Number(quantity);
    patch.quantity = Number.isFinite(cleanQuantity) && cleanQuantity > 0 ? cleanQuantity : null;
  }
  if (unit !== undefined) patch.unit = String(unit || '').trim() || null;
  if (alertThresholdPct !== undefined) {
    const cleanThreshold = Number(alertThresholdPct);
    patch.alert_threshold_pct = Number.isFinite(cleanThreshold) && cleanThreshold > 0 ? cleanThreshold : null; // 0 أو فاضي بيقفل التنبيه
  }
...
// ============================================================
// ============ تنبيهات حركة السعر اليومية لكل أصل (alert_threshold_pct) ============
// ============================================================
// لازم ينفّذ الـ migration بتاعه الأول (sql/portfolio-alerts.sql)

// ============ أقرب صورة مسجّلة من قبل بـ يوم واحد على الأقل (للمقارنة اليومية) ============
async function getYesterdaySnapshot(userId) {
  const cutoff = new Date(Date.now() - 20 * 60 * 60 * 1000).toISOString(); // آخر صورة قبل 20 ساعة على الأقل، عشان نضمن إنها "أمس" فعلاً حتى لو الكرون اتأخر شوية
  const { data, error } = await supabase
    .from('portfolio_snapshots')
    .select('assets_json, created_at')
    .eq('telegram_user_id', userId)
    .lte('created_at', cutoff)
    .order('created_at', { ascending: false })
    .limit(1)
    .maybeSingle();
  if (error) {
    console.error('getYesterdaySnapshot error:', JSON.stringify(error));
    return null;
  }
  return data || null;
}

// ============ بيرجع كل الأصول اللي عدّت حد التنبيه اللي المستخدم حدده لها مقارنة بأمس ============
export async function checkPortfolioPriceAlerts(userId) {
  const [{ assets }, reference] = await Promise.all([getPortfolio(userId), getYesterdaySnapshot(userId)]);
  if (!reference || !assets.length) return [];

  const prevByName = new Map((reference.assets_json || []).map((a) => [normalizeAssetName(a.name), Number(a.amount || 0)]));
  const triggered = [];
  for (const asset of assets) {
    const threshold = Number(asset.alert_threshold_pct);
    if (!Number.isFinite(threshold) || threshold <= 0) continue; // مفيش تنبيه مفعّل للأصل ده
    const prevAmount = prevByName.get(normalizeAssetName(asset.name));
    if (prevAmount === undefined || prevAmount <= 0) continue; // مفيش قيمة أمس نقارن بيها
...
// ============ صياغة رسالة تليجرام (HTML) لتنبيه حركة سعر أصل واحد ============
export function buildPortfolioAlertTelegramMessage(alert) {
  const arrow = alert.direction === 'up' ? '📈' : '📉';
  const sign = alert.changePct >= 0 ? '+' : '';
  const fmt = (n) => Number(n).toLocaleString('ar-EG', { maximumFractionDigits: 0 });
  return `${arrow} <b>${alert.name}</b> تحرك ${sign}${alert.changePct}% في يوم واحد\nالقيمة دلوقتي: <b>${fmt(alert.amount)} ج.م</b> (كان ${fmt(alert.prevAmount)} ج.م أمس)`;
}

// ============ صياغة payload تنبيه Push (Web Push notification) لحركة سعر أصل واحد ============
export function buildPortfolioAlertPushPayload(alert) {
  const arrow = alert.direction === 'up' ? '📈' : '📉';
  const sign = alert.changePct >= 0 ? '+' : '';
  const fmt = (n) => Number(n).toLocaleString('ar-EG', { maximumFractionDigits: 0 });
  return {
    title: `${arrow} ${alert.name} تحرك ${sign}${alert.changePct}%`,
    body: `القيمة دلوقتي: ${fmt(alert.amount)} ج.م`,

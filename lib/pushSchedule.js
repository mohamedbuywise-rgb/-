...
import { PREFERENCE_COLUMNS, LEGACY_PREFERENCE_COLUMNS, cleanPreferences, getNotificationPreferences, isMissingOptionalPreferenceColumn, isPushConfigured, sendClaimedPush, sendPushToUser } from './webPush.js';
...
// الإشعار المجدول لازم يوصل في ميعاده: لو الجهاز أوفلاين، الـ push service يرميه بعد ساعة بدل ما يوصل متأخر بساعات.
const SCHEDULED_PUSH_TTL = 3600;
...
const BADGE = './icons/badge-mono-96.png'; // شكل بسيط أبيض-شفافية عشان أندرويد يرسم أيقونة الإشعار الصغيرة صح بدل مربع أبيض
...
async function sendDailyReminder(userId, prefs, slot) {
  ...
  const result = await sendClaimedPush(userId, claimType, slot.dateKey, { ...payload, url: APP_URL, icon: ICON, badge: BADGE }, { urgency: 'high', ttl: SCHEDULED_PUSH_TTL });
  return result.sent ? 'sent' : (result.skipped || 'failed');
}
...
async function sendDailySummary(userId, prefs, slot) {
  ...
  const result = await sendClaimedPush(userId, 'daily-summary', yesterdayKey, {
  ...
  }, { urgency: 'high', ttl: SCHEDULED_PUSH_TTL });
  return result.sent ? 'sent' : (result.skipped || 'failed');
}
...
async function sendWeeklySummary(userId, prefs, slot) {
  ...
  const total = expenses.reduce((sum, expense) => sum + Number(expense.amount || 0), 0);
  const result = await sendClaimedPush(userId, 'weekly-summary', `${weekStartKey}:${slot.dateKey}`, {
...
async function sendPaymentReminders(userId, prefs, slot, ctx) {
  ...
  for (const r of list) {
    ...
    let body = '';
    try { body = plainText(buildReminderMessage(r.title, r.stage, r)); } catch { /* نرجع للنص البديل */ }
    // المفتاح فيه تاريخ الاستحقاق: القسط الشهري بيتنبّه كل شهر مش مرة واحدة بس في عمره
    const result = await sendClaimedPush(userId, 'payment-reminder', `${r.id}:${r.due_date}:${r.stage}`, {
      title: r.stage === 'overdue' ? 'دبّر — قسط فات ميعاده ⚠️' : 'دبّر — تذكير بموعد دفع 🗓️',
...
function makeContext() {
  ...
  return {
    async remindersFor(userId) {
      if (!byUser) {
        const rows = await getRemindersNeedingNotification({ extended: true }).catch((error) => {
...
// ---------- تنبيهات الاشتراك / التجربة (بتتنده من cron-daily، لأن بياناتها هناك) ----------
// بتتبعت مرة لكل (نوع + فترة) بس، وبس في ساعات النهار بتوقيت المستخدم (7 ص → 10 م) عشان الكرون بتاع منتصف الليل ما يصحّيهوش.
export async function sendSubscriptionAlertPush(userId, { kind, periodKey, title, body }) {
  if (!isPushConfigured()) return 'not_configured';
  const prefs = await getNotificationPreferences(userId);
  if (!prefs.subscriptionAlertsEnabled) return 'disabled';
  const { hour } = zonedParts(new Date(), prefs.timezone);
  if (hour < 7 || hour >= 22) return 'quiet_hours';
  const result = await sendClaimedPush(userId, `subscription-${kind}`, String(periodKey), {
    title,
    body,
    tag: `subscription-${kind}`,
    url: APP_URL,
    icon: ICON,
    badge: BADGE,
  });
...
async function getPreferencesMap(userIds) {
  ...
  for (let i = 0; i < userIds.length; i += ID_CHUNK) {
    ...
    if (error && isMissingOptionalPreferenceColumn(error)) {
      // قواعد البيانات القديمة لا تحتوي الأعمدة الاختيارية الجديدة؛ نكمل بباقي الإعدادات بدل تعطيل الجدولة كلها.
...
// ============ نقطة الدخول ============
// debug=true بيرجّع كمان تفاصيل لكل مستخدم (آخر 4 أرقام من الـ id فقط) توضّح ليه إشعار اتبعت أو لأ — لتشخيص الإشعارات.
export async function runPushSchedule({ now = new Date(), debug = false } = {}) {
  ...
  if (!userIds.length) return debug ? { ...summary, details: [], note: 'no_active_subscriptions' } : summary;
  ...
  for (const userId of userIds) {
    ...
    for (const task of TASKS) {
      ...
      if (slot) { jobs.push({ userId, prefs, slot, task }); dueNow.push(task.name); }
    }
    if (debug && details.length < 30) {
      const pad2 = (n) => String(n).padStart(2, '0');
      details.push({
        user: String(userId).slice(-4),
        hasPrefsRow: prefsMap.has(String(userId)),
        timezone: prefs.timezone,
        localTime: `${pad2(parts.hour)}:${pad2(parts.minute)}`,
        reminderTime: `${pad2(prefs.dailyReminderHour)}:${pad2(prefs.dailyReminderMinute)}`,
        dailyReminderEnabled: prefs.dailyReminderEnabled,
        dailyPresenceEnabled: prefs.dailyPresenceEnabled,
        dueNow,
      });
    }
  }
  ...
  await runWithConcurrencyLimit(jobs, CONCURRENCY, async ({ userId, prefs, slot, task }) => {
    try {
      ...
      if (debug && taskResults.length < 40) taskResults.push({ user: String(userId).slice(-4), task: task.name, result });
    } catch (error) {
      summary.failed += 1;
      console.error(`push ${task.name} failed for user ${userId}:`, error?.message || error);
      if (debug && taskResults.length < 40) taskResults.push({ user: String(userId).slice(-4), task: task.name, result: `error: ${String(error?.message || error).slice(0, 120)}` });
    }
  });
  return debug ? { ...summary, details, taskResults } : summary;
}

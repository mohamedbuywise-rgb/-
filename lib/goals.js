...
export async function getActiveGoals(userId) {
  const { data, error } = await supabase
    ...
    .eq('is_active', true)
    .eq('is_daily_piggybank', false) // الحصالة اليومية ليها عرض منفصل، مش بتتخلط مع أهداف الادّخار العادية
...
// ============================================================================
// ============ الحصالة اليومية — تسجيل/تتبع فقط، بدون أي تحويل بنكي فعلي ============
// ============================================================================
// ملحوظة مهمة: الرقم ده افتراضي بالكامل للتحفيز والمتابعة، مش خصم حقيقي من أي
// رصيد أو حساب بنكي — المستخدم بيدوس زرار وبس، من غير أي ربط مالي فعلي.

// ============ إنشاء حصالة يومية (مرة واحدة لكل مستخدم عادةً) ============
export async function createDailyPiggybank({ title, dailyAmount }, userId) {
  const cleanAmount = Number(dailyAmount);
  if (!cleanAmount || cleanAmount <= 0) return { error: 'محتاج مبلغ يومي صحيح أكبر من صفر.' };

  const { data, error } = await supabase
    .from('goals')
    .insert({
      telegram_user_id: userId,
      title: title || 'حصالة يوم بيوم',
      target_amount: 0, // مفيش هدف نهائي إلزامي، مجرد تجميع مستمر
      saved_amount: 0,
      is_daily_piggybank: true,
      daily_amount: cleanAmount,
      is_active: true,
    })
    .select('*')
    .single();
  ...
  if (error) {
    console.error('createDailyPiggybank error:', JSON.stringify(error));
    return { error: 'حصل خطأ وإحنا بننشئ الحصالة، جرب تاني.' };
  }
  return { piggybank: data };
}

// ============ الحصالة اليومية النشطة الحالية للمستخدم (لو موجودة) ============
export async function getDailyPiggybank(userId) {
  const { data, error } = await supabase
    .from('goals')
    .select('*')
    .eq('telegram_user_id', userId)
    .eq('is_daily_piggybank', true)
    .eq('is_active', true)
    .maybeSingle();
  if (error) { console.error('getDailyPiggybank error:', JSON.stringify(error)); return null; }
  return data;
}

// ============ إضافة "قرش النهاردة" — مرة واحدة بس في اليوم (بيتفحص بـ last_contribution_date)، والمبلغ ممكن يتغيّر كل مرة ============
export async function contributeDailyPiggybank(goalId, userId, amountOverride) {
  const today = new Date().toISOString().slice(0, 10);
  const { data: row, error: fetchError } = await supabase
    .from('goals')
    .select('id, saved_amount, daily_amount, last_contribution_date')
    .eq('id', goalId).eq('telegram_user_id', userId).eq('is_daily_piggybank', true)
    .maybeSingle();
  if (fetchError || !row) return { error: 'الحصالة دي مش موجودة.' };
  if (row.last_contribution_date === today) return { error: 'إنت أضفت قرش النهاردة بالفعل! تعالى بكرة 😊', alreadyDone: true };

  const cleanOverride = Number(amountOverride);
  const contribution = cleanOverride > 0 ? cleanOverride : Number(row.daily_amount || 0);
  const newSaved = Number(row.saved_amount || 0) + contribution;
  const updatePayload = { saved_amount: newSaved, last_contribution_date: today };
  // ============ لو المستخدم اختار مبلغ مختلف عن المبلغ الافتراضي، نخليه هو الافتراضي المقترح المرة الجاية ============
  if (cleanOverride > 0 && cleanOverride !== Number(row.daily_amount || 0)) updatePayload.daily_amount = cleanOverride;
  const { data, error } = await supabase
    .from('goals')
    .update(updatePayload)
    .eq('id', goalId).eq('telegram_user_id', userId)
    .select('*')
    .single();
...
export async function deleteDailyPiggybank(goalId, userId) {
  const { error } = await supabase.from('goals').delete().eq('id', goalId).eq('telegram_user_id', userId).eq('is_daily_piggybank', true);
  if (error) console.error('deleteDailyPiggybank error:', error);
}

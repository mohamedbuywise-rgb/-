...
// ============ التجربة المجانية ============
// trial_started_at تتسجل عند أول استخدام فعلي عبر lib/subscriptionAccess.js، وليس عند التسجيل أو /start.
...
export async function isInTrial(userId) {
  ...
  if (!startedAt) return false; // التسجيل أو /start وحدهما لا يبدآن التجربة؛ أول ميزة فعلية تفعلها بوابة الوصول.
...
// عدد المشتركين الفعّالين حاليًا (تاريخ الانتهاء في المستقبل) — رقم حقيقي فقط لعرضه في لوحة الشرف،
// مفيش أي تقريب أو تضخيم هنا؛ لو حابب تعرضه للمستخدمين لازم يكون هو نفسه الرقم اللي بيرجع من هنا.
export async function getActiveSubscribersCount() {
  ...
  if (error) {
    console.error('getActiveSubscribersCount error:', JSON.stringify(error));
    return null;
  }
  return count ?? null;
}

...
// ============ يبني ويبعت ملف PDF لأي تقرير (يومي / أسبوعي / شهري) + رسالة نص مختصرة قبله ============
export async function sendReportPdf({ chatId, title, periodLabel, expenses, comparisonLine, filename }) {
  ...
  caption += `\n📎 التفاصيل الكاملة جوه الملف المرفق`;
  ...
  try {
    ...
    await sendTelegramDocument(chatId, filename, pdfBuffer, null, 'application/pdf');
  } catch (err) {
    console.error('PDF generation/send failed:', err);
    await sendTelegramMessage(chatId, '⚠️ حصلت مشكلة في توليد ملف الـ PDF، بس التفاصيل فوق صحيحة.');
  }
}
...
export async function sendDataExport(userId, chatId) {
  ...
  const bom = '\uFEFF';
  const filename = `floosy-export-${new Date().toISOString().slice(0, 10)}.csv`;

  await sendTelegramDocument(chatId, filename, bom + csv, '📁 <b>اتفضل ملف بياناتك كامل</b>');
}

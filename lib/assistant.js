...
async function handleEntryDraft(userId, body, res) {
  ...
  // حماية إضافية (مش بس على مستوى البرومبت): "دين" لازم يكون معاه اسم علم حقيقي، مش كلمة عامة زي "حد"/"واحد"/"غير محدد"
  const GENERIC_DEBT_PERSON_WORDS = new Set(['حد', 'واحد', 'زول', 'شخص', 'غير محدد', 'مش معروف', 'unknown', 'someone']);
  const sanitizedTransactions = transactions.map((item) => {
    if (item?.type === 'debt' && GENERIC_DEBT_PERSON_WORDS.has(String(item.person || '').trim())) {
      // مش دين حقيقي — حوّلها مصروف عادي بدل ما نرفضها بالكامل (زي حالة "دليفري حد مني 80 جنيه")
      return { type: 'expense', amount: item.amount, currency_code: item.currency_code || 'EGP', category: 'مواصلات', note: item.note || item.raw_text || '', raw_text: item.raw_text };
    }
    return item;
  });
  let validTx = sanitizedTransactions.filter((item) => ((isFinancialEventType(item?.type) || item?.type === 'expense' || item?.type === 'purchase' || item?.type === 'asset') || item?.type === 'debt' || item?.type === 'portfolio_buy' || item?.type === 'portfolio_sell') && Number.isFinite(Number(item.amount)) && Number(item.amount) > 0 && (item.type !== 'debt' || item.person));
...
async function handlePortfolioAssetDetail(userId, body, res) {
  const assetId = body.assetId;
  if (!assetId) return res.status(400).json({ error: 'مفيش رقم أصل اتبعت.' });
  const result = await getPortfolioAssetDetail(userId, assetId);
  if (result.error) return res.status(400).json({ error: result.error });
  return res.status(200).json({ ok: true, asset: result.asset, unitPriceEgp: result.unitPriceEgp, history: result.history });
}
  ...
  try {
    ...
    switch (action) {
      ...
      case 'portfolio_asset_detail':
        return await handlePortfolioAssetDetail(userId, body, res);
      case 'receipt_scan':

...
async function classifyViaDecomposition(text, textNumbers) {
  let phrases = await segmentIntoPhrases(text, textNumbers.length);
  // لو التقسيم الذكي (LLM) فشل في إعطاء نفس عدد الأرقام، ننزل على المُقسّم الحتمي كخط دفاع
  // أخير مضمون رياضيًا بدل ما نستسلم ونرجع لتصنيف الرسالة كاملة مرة واحدة (اللي هو أصل مشكلة
  // "بلع كل البنود في معاملة واحدة" في الرسايل الطويلة).

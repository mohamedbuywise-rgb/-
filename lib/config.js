...
export const SUBSCRIPTION_PRICE_EGP = process.env.SUBSCRIPTION_PRICE_EGP || '199';
...
export const USAGE_LIMITS = {
  // text = كل تصنيف نصي مدفوع: إدخال يدوي، رسالة Telegram نصية، أو SMS بنكية.
  // الفويس لا يستهلك هذا العداد؛ له عداد voice مستقل لأن التفريغ نفسه مكلف.
  // statement = رفع كشف حساب بنكي (PDF/صورة) — محدود بشدة لأنه ممكن يحتوي عشرات المعاملات
  // في استدعاء واحد، فالتكلفة الفعلية لكل "استخدام" أعلى من فويس أو نص عادي.
  paid: { voice: 250, ocr: 50, chat: 180, text: 300, statement: 10 },
  // التجربة المجانية دلوقتي حدودها *يومية* مش إجمالية على الـ3 أيام (شوف resolveUsageContext في
  // rateLimits.js فين بيتحدد الـ period اليومي): فويس 15 وكتابة/نص 15 وفاتورة (ocr) 3 كل يوم،
  // ولما الـ3 أيام تخلص تقفل التجربة تمامًا (isInTrial بيرجع false) ولازم اشتراك.
  trial: { voice: 15, ocr: 3, chat: 50, text: 15, statement: 2 },
};
...
// ============ حدود رفع كشف حساب بنكي (PDF/صورة) — حماية إضافية فوق عداد statement الشهري ============
// كشف حساب طويل جدًا (سنة كاملة مثلاً) بيكلّف أكتر بكتير من كشف شهر عادي لأنه بيتقسم استدعاءات AI
// أكتر، فبنحط سقف صفحات وسطور تحمي التكلفة لكل رفعة واحدة، بغض النظر عن عداد الاستخدام الشهري.
export const BANK_STATEMENT_MAX_PDF_PAGES = 12;
export const BANK_STATEMENT_MAX_LINES = 400;
export const BANK_STATEMENT_LINES_PER_AI_CALL = 60; // نفس حجم الدفعة المستخدم في استيراد CSV العادي

...
export function buildReportHtml({ title, periodLabel, generatedAt, total, count, topCategoryName, comparisonLine, categories, flow }) {
  ...
  const flowRowsHtml = (flow?.items || [])
    .map((it) => {
      const isIn = it.direction === 'borrowed';
      const dateLabel = it.created_at ? new Date(it.created_at).toLocaleDateString('ar-EG', { day: 'numeric', month: 'short' }) : '';
      return `
      <div class="item-row">
        <span class="item-amount" style="color:${isIn ? '#0E8C57' : '#B4402F'}">${isIn ? '+' : '-'}${esc(it.amount)} ${esc(currencyLabel(it.currency_code))}</span>
        <span class="item-desc"><span class="item-date">${esc(dateLabel)}</span> — ${esc(isIn ? 'واصل من' : 'واصل لـ')} ${esc(it.personName || it.note || '')}</span>
      </div>`;
    })
    .join('');

  const flowHtml = flow && (flow.in > 0 || flow.out > 0)
    ? `
      <div class="group-title">حركة السيولة (الشهر ده)</div>
      <div class="row-stats" style="margin-bottom:12px;">
        <div class="card-stat"><div class="l">داخل (واصل من)</div><div class="v" style="color:#0E8C57;">${esc(flow.in)} ج.م</div></div>
        <div class="card-stat"><div class="l">خارج (واصل لـ)</div><div class="v" style="color:#B4402F;">${esc(flow.out)} ج.م</div></div>
        <div class="card-stat total"><div class="l">صافي الحركة</div><div class="v">${esc(flow.net)} ج.م</div></div>
      </div>
      <div class="cat-card">${flowRowsHtml || '<div class="item-row"><span class="item-desc">مفيش حركة سيولة متسجلة الشهر ده.</span></div>'}</div>`
    : '';
